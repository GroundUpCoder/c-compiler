#!/usr/bin/env node
// tools/liabilities/idspace.js — the cross-ref id allocator for docs/ (docs/archive/0358).
//
...
// file queue, but the archive (docs/archive/) still occupies that namespace and
// the survey tests exercise the machinery through it.
//
// Both id spaces in docs/ are (were) allocated by LANES: a lane branches,
// files a ticket (`docs/NNNN-*.md`) and/or a liability entry (`### Lnn` in
// docs/LIABILITIES.md), and pushes at the END. So the highest id visible on
...
//
// This module answers the question the shell one-liner in docs/archive/0358 answers,
// but reading TREES rather than diffs, which is load-bearing: the obvious
// cheap form
//
//     git log --all -p -- docs/LIABILITIES.md | grep '^+### L'
//
...
//
// The survey answers "what ids exist?"; docs/archive/0360 added the second question a
// caller needs and never asked — "how stale is what I just surveyed?". See the
// freshness section below.
//
// Zero dependencies (Node built-ins only), matching tools/liabilities/liabilities.js.
'use strict';
...
// freshness section (docs/archive/0360).
...
// are simply omitted (an old branch predating docs/, a tag from before the
...
// The same object may sit at several historical paths (a rename moves the
// tree without changing its oid); fold every path's hits into one map.
function unionAtRefs(root, refs, relPaths) {
  const out = new Map();
  for (const rel of relPaths) {
    for (const [oid, at] of objectsAtRefs(root, refs, rel)) {
...
// ---------- how stale is the survey? (docs/archive/0360) ----------
//
...
// thesis of docs/LIABILITIES.md. So the survey now MEASURES its staleness:
//
...
// coordination point (a pushed reservation ref), which is docs/archive/0364, register
...
const PROBE_TIMEOUT_MS = 5000;          // an allocator that hangs on a flaky
                                        ...
                                        // honestly stale (docs/archive/0360).
...
function computeFreshness(root, opts) {
  ...
  if (probe.status === 'none') {
    ...
    line = `freshness: ${probe.remote} advertises nothing this survey has not seen (git ls-remote) — ` +
      'a fetch would move nothing. Unpushed ids in another CLONE remain invisible (docs/archive/0364).';
...
// ref until it commits, and that is the incident that filed docs/archive/0360: an
...
function worktreeTicketIds(wtPath) {
  const ids = [];
  // docs/archive is the live location; the two todos/ forms are what a sibling
  // worktree still on a pre-2026-09-21 branch has.
  for (const dir of [path.join(wtPath, 'docs', 'archive'), path.join(wtPath, 'todos'), path.join(wtPath, 'todos', 'done')]) {
    let ents;
...
function worktreeLiabilityIds(wtPath) {
  let text;
  try { text = fs.readFileSync(path.join(wtPath, 'docs', 'LIABILITIES.md'), 'utf8'); }
  catch { try { text = fs.readFileSync(path.join(wtPath, 'todos', 'LIABILITIES.md'), 'utf8'); } catch { return []; } }
  const ids = [];
...
function surveyTickets(root, localIds) {
  ...
  // Both homes of the archive: docs/archive since the 2026-09-21 layout cleanup,
  // todos/ (open items + done/) on every ref that predates it.
  const byTree = unionAtRefs(root, refs, ['docs/archive', 'todos']);
  const ids = new Map();
  for (const [tree, treeRefs] of byTree) {
    let listing;
    try { listing = git(root, ['ls-tree', '-r', '--name-only', tree]); }
    catch (e) { throw new IdSpaceError(`cannot list the archive at ${treeRefs[0]}: ${e.message.trim().split('\n')[0]}`); }
...
function surveyLiabilities(root, localIds) {
  const refs = refNames(root);
  const byBlob = unionAtRefs(root, refs, ['docs/LIABILITIES.md', 'todos/LIABILITIES.md']);
  const ids = new Map();
...
// the network probe — and says THAT too (docs/archive/0360).
