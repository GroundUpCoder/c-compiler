#!/usr/bin/env node
// tools/liabilities/liabilities.test.js — tests for the liability register's validator
// (docs/archive/0286). Zero-dependency; run with:  node tools/liabilities/liabilities.test.js
//
...
test('RED: an entry whose ticket does not exist fails', () => {
  const res = run(['### L01 — a gap\n- ticket: 9999\n- file: src/thing.c\n- anchor: int x;']);
  assert.match(messages(res), /ticket 9999 has no docs\/archive\/9999-\*\.md file/);
});
...
test('RED: a deferral target that is not a todo at all fails', () => {
  ...
  assert.match(messages(res), /defers-to 9999 has no docs/);
});
...
test('the checked-in register passes its own check', () => {
  ...
  // real register is the `liabilities-check` case of tests/liabilities/run.js.
...
test('the register still demonstrates the pinned-expired-deferral shape (L20, L21)', () => {
  // Of the two 0286-era motivating findings, L10 (né docs/archive/0291) CLOSED at
  // the #99 CD34 fold — the register's designed success path: the funded
  // ticket landed and the entry left in the same commit. L20 (né docs/archive/0300)
  // lives on, and L21 is a second live instance of the demonstrated shape (a
  // pinned expired deferral to an ARCHIVED target), so the demonstration
  // stays two-deep. offline: judged from docs/archive/, no cc probe needed.
  const res = LIB.check({ offline: true });
  assert.ok(!res.entries.some(e => e.id === 'L10'),
    'L10 returned — its gap closed at the #99 fold; a re-add needs a NEW gap and a new id');
  for (const id of ['L20', 'L21']) {
  ...
  assert.ok(res.pinned.some(p => p.id === 'L21'), 'L21 does not fire as a pinned expired deferral');
});
...
test('unpinning the real entries turns the real register RED', () => {
  ...
  // deferral-outlived-its-premise shape for L20 (né 0300) and L21 (L10, the
  // other 0286 motivator, closed at the #99 fold).
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'liab-'));
  const registerPath = path.join(root, 'REG.md');
  const real = fs.readFileSync(path.join(__dirname, '..', '..', 'docs', 'LIABILITIES.md'), 'utf8');
...
test('citedFiles exposes the register\'s files to the diff planner', () => {
  ...
  assert.ok(cited.files.includes('docs/WIN32.md'), 'docs/WIN32.md should be cited');
});

// ---------- cross-ref Lnn allocation (docs/archive/0358) ----------
//
...
// CLI's REPO_ROOT is the temp tree (tools/liabilities/ two levels below it,
// the register at docs/LIABILITIES.md). Returns { root, tools, reg, git }.
function repo() {
  root = fs.mkdtempSync(path.join(os.tmpdir(), 'liab-refs-'));
  const tools = path.join(root, 'tools', 'liabilities');
  fs.mkdirSync(tools, { recursive: true });
  fs.mkdirSync(path.join(root, 'docs'), { recursive: true });
  const reg = path.join(root, 'docs', 'LIABILITIES.md');
  const git = (...args) => execFileSync('git', args, { cwd: root, encoding: 'utf8' }).trim();
  git('init', '-q');
  git('config', 'user.email', 't@t');
  git('config', 'user.name', 't');
  for (const f of ['liabilities.js', 'idspace.js']) {
    fs.copyFileSync(path.join(__dirname, f), path.join(tools, f));
  }
  return { root, tools, reg, git };
}

function nextId(root, tools, args = []) {
  return execFileSync('node', [path.join(tools, 'liabilities.js'), 'next-id', ...args],
    { cwd: root, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
}

test('RED: two lanes off a common base get DISTINCT liability ids', () => {
  const { root: r, tools, reg, git } = repo();
  fs.writeFileSync(reg, REG(['### L01 — first\n- ticket: 0001']));
  git('add', '-A'); git('commit', '-q', '-m', 'base');
  const base = git('rev-parse', 'HEAD');

  git('checkout', '-q', '-b', 'lane-a');
  const a = nextId(r, tools);
...
test('the liability allocator REFUSES outside a repo, and --local opts out', () => {
  ...
  fs.writeFileSync(path.join(root, 'docs', 'LIABILITIES.md'), REG(['### L07 — g\n- ticket: 0001']));

  let failed = null;
  try { nextId(root, tools); } catch (e) { failed = e; }
  ...
  assert.match(nextId(root, tools, ['--local']), /^L08\s+liability id L08 — WORKING TREE ONLY/);
});
