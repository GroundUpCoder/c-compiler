#!/usr/bin/env node
// tools/liabilities/idspace.test.js — the freshness half of the cross-ref id allocator
// (docs/archive/0360). Zero-dependency; run with:  node tools/liabilities/idspace.test.js
//
...
function writeTicket(wt, id) {
  fs.mkdirSync(path.join(wt, 'docs', 'archive'), { recursive: true });
  fs.writeFileSync(path.join(wt, 'docs', 'archive', `${id}-x.md`), `# ${id} — x\n\n- **Status**: open\n`);
}

function writeRegister(wt, ids) {
  fs.mkdirSync(path.join(wt, 'docs'), { recursive: true });
  fs.writeFileSync(path.join(wt, 'docs', 'LIABILITIES.md'),
    '<!-- BEGIN ENTRIES -->\n\n' +
...
// remote and no local signal says so — this is the state docs/archive/0360 is about.
