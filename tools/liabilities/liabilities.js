#!/usr/bin/env node
// tools/liabilities/liabilities.js — parser + validator for the liability register (docs/archive/0286).
//
// The register (docs/LIABILITIES.md) records gaps that the tree DESCRIBES but
...
//
//   node tools/liabilities/liabilities.js check    # validate; exit 1 on any failure
...
//   NNNN   a legacy file-queue id, resolved against docs/archive/ (the archive)
...
// The retired file queue's completed items (todos/done/ until the 2026-09-21
// layout cleanup) — legacy NNNN ids resolve here.
const ARCHIVE_DIR = path.join(REPO_ROOT, 'docs', 'archive');
const REGISTER_REL = 'docs/LIABILITIES.md';
...
// resolved against the filesystem. Since the 2026-07-30 cutover there is no
// open-items directory at all, so `open` is always empty; it stays in the
// shape because callers and tests address both states.
function scanTickets() {
  ...
  return { open: new Map(), done: pick(ARCHIVE_DIR) };
}
...
function check(opts) {
  ...
  // (closed — cc done/dropped, or a legacy id in docs/archive/), 'missing', and
    ...
    switch (state(e.ticket)) {
      case 'done':
        errors.push(CC_RE.test(e.ticket)
          ...
          : `${at}: ${where(e.ticket)} is CLOSED (docs/archive/) — this gap's funding is gone: legacy ids cannot fund a gap since the 2026-07-30 cutover; re-point it at a live cc ticket (#N)`);
        break;
      case 'missing':
        errors.push(CC_RE.test(e.ticket)
          ? `${at}: ${where(e.ticket)} does not exist in the c-compiler cc project`
          : `${at}: ticket ${e.ticket} has no docs/archive/${e.ticket}-*.md file`);
        break;
    }
    ...
    for (const id of defers) {
      ...
      if (st === 'missing') {
        errors.push(CC_RE.test(id)
          ...
          : `${at}: defers-to ${id} has no docs/archive/${id}-*.md file`);
        continue;
      }
...
const refLabel = (ref) => CC_RE.test(String(ref)) ? String(ref) : `docs/${ref}`;
...
function cmdList(argv) {
  ...
  if (errors.length) {
    process.stderr.write(`\n(${errors.length} error(s) — run: node tools/liabilities/liabilities.js check)\n`);
    process.exit(1);
  }
}

// The next free `Lnn`, derived across every ref (docs/archive/0358). The register had
...
function cmdNextId(argv) {
  ...
  const offline = argv.includes('--offline');   // skip the freshness probe (docs/archive/0360)
...
const USAGE = `liabilities.js — the liability register's validator (docs/archive/0286)

  check [--offline]
                   validate docs/LIABILITIES.md; exit 1 on any failure.
  ...
  next-id [--local] [--offline]
                   ...
                   (docs/archive/0358), with a freshness verdict on what it surveyed (docs/archive/0360)
`;
