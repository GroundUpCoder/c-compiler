...
const CFG = (() => {
  ...
  function makeFunctionReducible(cfgFn) {
    ...
    // Build (nodes, adj) for Tarjan: block → its successors.
    const buildAdj = () => {
      const adj = new Map();
      for (const b of cfgFn.blocks) {
        adj.set(b, b.terminator ? b.terminator.successors : []);
      }
      return adj;
    };

    // Find the "entry blocks" of an SCC: blocks in the SCC with at least
    // one predecessor that's NOT in the SCC. (External predecessors are
    // the things that make this block reachable from outside the loop.
    // A natural loop has exactly one such block; irreducible has > 1.)
    ...
    // One dispatcher per multi-entry SCC. Each insertion strictly reduces
    // the count of irreducible SCCs by exactly one (the transformed SCC
    // becomes single-entry; no new irreducibility is created elsewhere).
    // So the loop terminates after exactly the initial irreducible-SCC
    // count — no bound, no guard, no "what if it loops forever." If this
    // loop ever doesn't terminate, the algorithm is wrong and we want
    // the hang as a visible bug, not a silent partial output.
    while (true) {
      const sccs = tarjanSCC(cfgFn.blocks, buildAdj());
      let target = null;
      for (const sccArr of sccs) {
        // SCCs of size one are always reducible
        if (sccArr.length === 1) {
          continue;
        }
        const sccSet = new Set(sccArr);
        const entries = entriesOf(sccSet);
        if (entries.length <= 1) continue;
        target = { sccSet, entries };
        break;
      }
      if (!target) break;
    ...
    const buildBody = (cfgFn) => {
      ...
      const liftBlock = (block) => {
        ...
        if (term instanceof CFG.Br) {
          ...
          let elseBranch = new AST.Block(null, [
          ...
          for (let i = term.targets.length - 1; i >= 0; i--) {
            ...
            elseBranch = new AST.Block(null, [new AST.If(
              term.loc, cond,
              new AST.Block(null, thenStmts),
              elseBranch,
            )]);
          }
