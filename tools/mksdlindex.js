...
const ABSENT = [
  ...
  // SDL_LoadWAV left this list at #723 (the two-sided edit): the complete
  // pinned-upstream WAV decoder is real now — see the audio section note.
  { label: 'SDL_mixer (`Mix_*`) and `SDL_LoadWAV_IO` do not exist — load WAV files with SDL_LoadWAV (path form; there is no public SDL_IOStream), convert with SDL_CreateAudioStream, mix with SDL_MixAudio, and push PCM through SDL_PutAudioStreamData.',
    absent: ['Mix_OpenAudio', 'SDL_LoadWAV_IO'],
    see: ['SDL_LoadWAV', 'SDL_PutAudioStreamData', 'SDL_CreateAudioStream', 'SDL_MixAudio'] },
  { label: 'Window management: `SDL_MinimizeWindow`, `SDL_MaximizeWindow`, `SDL_SetWindowFullscreen` — the WM owns placement and chrome.',
    absent: ['SDL_MinimizeWindow',
