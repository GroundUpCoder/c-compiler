...
- **Question**: port `codex exec`, or write a native gucOS client on the same
  wire protocol.
- **Inputs**: `docs/archive/0418` §Result, `docs/archive/0417`, `docs/archive/0442`,
  ticket `#190` (0445) and its report
  `~/git/meta/gucos/notes/rust-codex-feasibility-2.md`, ticket `#191` (0446),
  the codex source at `2e1607ee2f`, `docs/RUST.md` §1 and §3.
...
- **The TLS stack leaves the graph by architecture, not by work.** Before the
  ...
  puts TLS in the host (`docs/archive/0417` fds; the 0442 shim absents
...
1. **"The 0442 shim covers what codex calls."** HOLDS, as scoped, at the
   compile level. The shipped split (`docs/archive/0442` §Result) is: 36 real
...
`block_in_place` sites (`docs/archive/0418` §6.7 — re-counted, exactly 2).
...
- **Risks, in order**: (1) the tokio single-thread runtime has no runtime
  ...
  `docs/archive/0418` §8, bounded by the cut surface; (5) MCP: `rmcp` is the
...
- **T2 — codex as a host-side service**, bridged into the OS. **REJECTED**:
  every gucOS binary is a wasm module inside the OS (`docs/OS.md` north
