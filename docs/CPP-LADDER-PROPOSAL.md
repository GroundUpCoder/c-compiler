...
`os/os-common.js:353+` (docs/archive/0118): **`overlay@1`** manifests fold
...
serve.js overlay support landed as docs/archive/0141. gucman packages (`tools/
...
| **DOSBox** (GPL2) | C++ x86/DOS emulator, ~130 kloc | moderate STL, C-style core, SDL front-end | **High** — size, audio/timing, SDL1→our subset | A DOS catalogue in a window. NB the emulator family has its own track — see `docs/EMULATORS.md` (routing note below) |
...
- **Emulator cores (Snes9x / VBA-M / DeSmuME)** — routed, not shelved: the
  emulator ladder is `docs/EMULATORS.md` (SameBoy, the `br_table` A/B and
