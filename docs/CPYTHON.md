...
Status: **DESIGN, ratified route — EXECUTED by `docs/archive/0340` on 2026-07-28.**
...
> 🔴 **HOW TO RESOLVE THE `docs/NNNN` POINTERS IN THIS DOCUMENT — read this
> before following one.** Ticket numbering moved into the cc ticket DB on
> 2026-07-30 (cutover `416fb45c`; `docs/queue.js` is deleted) and **the numbers
> did NOT carry over.** The legacy `NNNN` now survives only inside the ticket's
> *title string*.
>
> - A **closed** item still resolves as a file: `docs/archive/NNNN-*.md`.
...
> |---|---|---|
> | `docs/archive/0052` | **#3** | loopback AF_INET |
> | `docs/archive/0054` | **#7** | AF_INET relay transport |
> | `docs/archive/0325` | **#124** | libc surface gaps (Group D) |
> | `docs/archive/0331` | **#128** | ship the gucman package |
> | `docs/archive/0336` | **#131** | CPython startup 26× clang |
> | `docs/archive/0346` | **#135** | SCOPING ONLY: price Tcl + Tk (tkinter) |
...
overturn. Implementation is `docs/archive/0340` (vendor tree + stdlib + expanded
binary) then `docs/archive/0331` (the package + e2e), in that order.
...
**shared with the eventual our-compiler CPython build** (gated on `docs/archive/0336`
...
| `ensurepip/` | (b) 1.8 MB, mostly a bundled pip wheel; pip needs `_ssl` + networking that gucOS does not have (§3.3) — revisit with the network stack (`docs/archive/0052`/`0054`) | 1.8 MB |
...
Filed as a follow-up idea inside `docs/archive/0340`, to be picked up only if
...
- `termios.c`: our libc termios lacks `tcsendbreak`/`tcdrain`/`tcflush`/
  ...
  work, now recorded as `docs/archive/0325` Group D**. Deferred from the probe;
...
|---|---|---|
| `ELOOP` in libc `errno.h` | `pathlib`, `zipfile`, `zipapp`, `compileall` (`errno.py` re-exports; `from errno import ELOOP` throws) | **one `#define ELOOP 40`** matching the kernel's live numbering (`host.js:10687`; BlockFS really raises it, SYMLOOP_MAX=40 walk) + `strerror` line → `docs/archive/0325` Group D. Reaches cc2wasm only after the `docs/archive/0330` libc re-vendor (206-commit staleness) or interim via the build shim (`python-clang-shim.h` precedent). |
...
| `_socket` | `socket`, `asyncio`, `socketserver`, `ftplib`, `smtplib`, `poplib`, `imaplib`, `mailbox` | OUT of M1 — `socketmodule.c` needs `netinet/*` (one of M0's five non-parsing TUs) and gucOS networking is AF_UNIX + the open `docs/archive/0052`/`0054` AF_INET items. A socket-module port is its own item; **asyncio not shipping-functional is the single biggest honesty flag on "real CPython"** — say it wherever the package is described. The `.py` trees ship per the §2 rule and light up with the port. |
...
sibling libc (**dies when `docs/archive/0330` re-vendors** — 0340 should sequence
after 0330 precisely so the shim shrinks to nothing). Our-compiler-only:
`bin.json` with `__minstack(8388608)`; blocked on `docs/archive/0336` as ruled, plus
...
```jsonc
{
  ...
  "files": {
    "bin/cpython-clang.wasm": { "nativeApp": "python-clang" },  // sibling overlay app name — still `python-clang` until the sibling manifest renames (docs/archive/0374 merge-time step); flip in the same change window as that overlay republish
...
docs/archive/0383, the deferred 0374 merge-time step, in the same change window as
...
- `docs/archive/0331` (ship the package): **unblocked in principle** — its blocker
  ("a committed CPython vendor tree", "M1 unfunded") is dissolved by this
  design + `docs/archive/0340`. It stays sequenced after 0340 and consumes §6
  verbatim. Its "python3/cpython reserved / must not be claimed here" line is
  superseded by §6.2's pending call.
- `docs/archive/0313`'s M1: the ladder's M1 is now **M1-clang** (this design; the
...
- MicroPython: unaffected. It remains the lightweight scripting Python and
  ...
  entries, zero `python*` hits". That was measured before `docs/archive/0338`, and
