...
`docs/archive/0149` (the scheme + verb remap) and `docs/archive/0150` (emacs bindings in GUI
...
1. **Cmd+arrows are line/doc nav in the macOS scheme** (⌘←/→ =
   ...
   The keybinding-override grab table (docs/KEYBINDING-OVERRIDE-SYSTEM.md)
...
2. **Redo (⌘⇧Z / Ctrl+Y) is not implemented** — `os/keys.h` binds KA_UNDO
   only. EDIT has a single-level undo buffer since `docs/archive/0135` (the Win95
