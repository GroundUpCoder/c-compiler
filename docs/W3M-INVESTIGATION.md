...
transport (the `__http_*` kernel RPC family, docs/archive/0172, behind the
`os/curl/` libcurl-easy veneer, docs/archive/0173) instead of raw TCP
...
- **There are NO raw sockets.** Tier-1 loopback AF_INET (`docs/archive/0052`) is
  still OPEN in the queue; the arbitrary-host relay (`docs/archive/0054`) is
...
- **CORS is a first-class runtime limit (browser tab).** `fetch()` from
  ...
  (`docs/NETWORK.md:44`). Headless `boot.js` (Node fetch) is
  unrestricted. This is the crux of the value question (see Verdict).
- **`__http_*` transfers ARE file descriptors since docs/archive/0417** (this
...
1. **CORS makes an in-browser text browser marginal until Tier-4 lands.**
   ...
   (`docs/NETWORK.md`). A web browser that can only load same-origin
   ...
   relay, **`docs/archive/0054`, which is still open and itself blocked on the
...
**What would unblock a GO:** land the Tier-4 relay (`docs/archive/0054`, and its
...
- gucOS in-tree: `kernel.js` (0x06xx HTTP RPCs, `_httpXfers`), `os/curl/` (veneer + `__http_*` imports), `os/term/term.c` (VT capabilities), `os/kernel-worker.js`/`os/boot.js` (`TERM=xterm-256color`), `os/win32/kernel32.c` (`CreateThread` stub), `docs/NETWORK.md` (tier model + CORS)
