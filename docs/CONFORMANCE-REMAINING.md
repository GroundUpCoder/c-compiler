...
- ~~**SDL keysym for shifted letters**~~ — **FALSE POSITIVE, dropped** (the
  ...
  `docs/SDL3.md`). SDL3 keycodes are *modifier-applied* — upstream computes
...
- ~~**Volatile accesses vs the inliner**: `twice(mmio)` inlines to two volatile
  ...
  FIXED 2026-07-15 (docs/archive/0187): EIdent / `*p` / `a[i]` / `s.f` / `p->f`
...
- ~~**Residual `longjmp` in non-statement position** (`x ? longjmp(b,1) : ...`,
  ...
  FIXED 2026-07-27 (docs/archive/0312), and SUPPORTED rather than diagnosed: C11 places
...
- ~~**setjmp contexts required by C11 7.13.1.1p4 but rejected**:
  ...
  FIXED 2026-08-03 (ticket #117 / docs/archive/0311). The lowering now accepts the
...
1. **AST invariant checker between passes** (test builds only): every
   ...
   Complements `docs/GOTO-LABELS-AST-REFACTOR.md` (which stays worth doing).
