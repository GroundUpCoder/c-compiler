...
Design doc for the kernel layer under `docs/OS.md` Phase 1. Read that first
...
(`docs/archive/0001`): asynchronous signal delivery at libc safe points
...
ordered exit handshake (OP.EXIT). Phase 3 (`docs/archive/0002`): the Tty
...
amendment is implemented (`docs/archive/0009`): kernel-owned per-process fd
...
(`docs/archive/0003`): pipes as OFDs — `PIPE_CREATE` + kernel-side buffers
...
shell-port acceptance gate — **PASSED** (`docs/archive/0005`): busybox hush
runs as /bin/sh with zero kernel workarounds.

**Ptys are implemented** (`docs/archive/0020`, brokered mode only): the
...
- `setpgid`, `getpgid`, `setsid`, `getsid` (0x000A, docs/archive/0043 — pgrep
  wanted it), plus honoring the already-plumbed `POSIX_SPAWN_SETPGROUP`.

### /proc — the synthetic procfs (docs/archive/0043, landed 2026-07-09)
...
## The spawn path: compiled-Module cache (docs/archive/0037, landed 2026-07-09; generalized to rw volumes by #188, 2026-07-30)
...
## The spawn path: shebang exec (docs/archive/0065, landed 2026-07-10)
...
## strace — per-pid syscall-RPC trace (docs/archive/0046, landed 2026-07-10)
...
0x00xx process   SPAWN WAIT KILL EXIT SETPGID GETPGID SETSID SIGDISP SIGMASK GETSID
                 SETITIMER GETITIMER (docs/archive/0044 — ITIMER_REAL only, ms wire)
...
0x03xx misc      COMPILE (the existing /bin/cc hook); CLIP_SET/CLIP_GET
                 (docs/archive/0090): ONE kernel-held clipboard slot {fmt, bytes}
...
0x05xx sockets   SOCK_SOCKET/BIND/LISTEN/ACCEPT/CONNECT/PAIR/SHUTDOWN —
                 AF_UNIX control plane (docs/archive/0008; data plane rides
                 FS_READ/FS_WRITE/FS_CLOSE/FS_SELECT, see below)
0x1xxx WM        SURFACE_CREATE/DESTROY/SET_TITLE/CONFIGURE (docs/WM.md;
                 present is deliberately NOT an RPC — pure SAB flip/seq,
                 mailbox; 0x1004 reserved should damage tracking ever want
                 one; CONFIGURE = the client's resize ack, docs/archive/0019)
0x2xxx audio     AUDIO_OPEN/AUDIO_CLOSE (docs/archive/0017; WM.md "Audio mixing" —
                 PCM rides the source-ring SABs and the one page-owned
                 output ring, never RPCs); AUDIO_GAIN (docs/archive/0048): master
...
### Interval timers (docs/archive/0044, landed 2026-07-09)
...
- ~~One tty in v1 ... pty pairs wait until something needs them~~ — pty
  pairs landed with their consumer (`docs/archive/0020`, the WM's terminal
...
## Pipes and cross-process blocking (implemented, docs/archive/0003)
...
## AF_UNIX sockets (implemented, docs/archive/0008)
...
### Kernel-owned endpoints (docs/archive/0014)
...
Since docs/archive/0168 `peer.send()` also KICKS the client's input ring
...
ring at entry or its wait resolves. Since docs/archive/0178 the kick serves the
...
- **Publish, don't serve** — seqlock vDSO page for kernel-written,
  ...
  (docs/archive/0179, landed 2026-07-14): a 12-word tail block on the kernel page
...
- **Immutable data serves itself** — the sealed /usr volume read
  process-side by host.js's own BlockFS reader (docs/archive/0180, landed
  ...
  brokered (the kernel owns EROFS-after-the-walk, docs/archive/0040); relative
...
- **SPSC data planes** — one-producer/one-consumer rings (input ring,
  audio rings, framebuffers; pipes since docs/archive/0181, landed 2026-07-14):
...
- **Waits**: two tiers by rule — raw SAB futex for single-source waits
  (vsync, sleeps, the input ring); the kernel WAIT RPC (docs/archive/0178) is
...
## HTTP transport (0x06xx; implemented, docs/archive/0172; fd-shaped, docs/archive/0417)
...
First consumer: the libcurl veneer (docs/archive/0173); the primitive is also the
...
- **A transfer IS an open file description** (kind `http`, the FS_WATCH
  precedent — docs/archive/0264). `HTTP_BODY` stages an optional request body
...
- **Two kernel deadlines bound every transfer** (docs/archive/0417): a HEADERS
...
`duplex:'half'`, Chromium-only) and WebSockets (docs/archive/0440) would be NEW
...
## WM extension (designed: `docs/WM.md`, 2026-07-07; WM client landed, docs/archive/0014)

Per `docs/OS.md` Phase 3, the compositor rides this same control plane:
...
`docs/WM.md`. The analogy is direct — focused surface : input routing ::
...
## The ksvc service seam — the kernel's C half (2026-07-22, docs/archive/0275)
...
Design + ABI details: `docs/archive/0275-kernel-text-service-design.md`.

## The vsync broadcast (2026-07-10, docs/archive/0100)
...
real frame clock (the compositor rAF, docs/archive/0055). 0100 exports it:

- Four tail words on the kernel page (see the layout comment in kernel.js —
  the payload cap stops 64 bytes short of the page end since docs/archive/0179
  ...
  `KP_VSYNC_SEQ`, a tick counter; and — since docs/archive/0169 (the on-demand
...
- `KernelClient.vsyncWait()` parks on the word (`Atomics.waitAsync`),
  ...
  docs/archive/0167, IDLE-POWER Stage 1); the frame-loop driver needs no change.
...
## The fd/data-plane amendment (2026-07-06, docs/archive/0009)
...
as they are; the brokered path is the OS's. (Since docs/archive/0026 the OS's
...
4. **Job control + pipes**: setpgid/setsid/tcsetpgrp, stop/cont, SIGTTIN;
   pipe wait-queues + SIGPIPE. DONE (docs/archive/0003; setpgid/getpgid RPCs
   exist kernel-side — the thin libc wrappers land with the shell port that
   needs them).
5. **Acceptance: the shell port** — PASSED (docs/archive/0005): busybox hush
...
6. **AF_UNIX sockets** (docs/archive/0008): the 0x05xx control plane over the
