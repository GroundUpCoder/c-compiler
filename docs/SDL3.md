...
- **Video is already 100% WebGPU** (software blitter + batched 2D renderer in
  ...
  binding is **deferred until JSPI reaches iOS** (see `docs/WEBGPU.md` →
...
- **Timer:** `SDL_GetTicks` (true `Uint64` ms since `SDL_Init`), `SDL_Delay`
  ...
  park, headless Node blocks like usleep (docs/archive/0224); ONLY the standalone
...
- **`SDL_Delay`** fails loud instead of silently no-opping (was a silent no-op
  ...
  routes around it. (Since docs/archive/0224 the throw is scoped to the standalone
...
- **`SDL_WaitEvent` / `SDL_WaitEventTimeout` are a REAL block** (docs/archive/0161,
...
- ~~**Full relative-mouse mode (Pointer Lock)**~~ — landed 2026-07-08
  (docs/archive/0018): `SDL_SetWindowRelativeMouseMode` / `SDL_GetWindowRelativeMouseMode`
...
(Landed 2026-07-08, docs/archive/0019: WM-initiated window resize —
...
plus `SDL_GetWindowSize`. Landed 2026-07-11, docs/archive/0089:
...
on-demand now — both done; see `docs/WEBGPU.md`.)
...
Have (since docs/archive/0161): `SDL_WaitEvent`/`SDL_WaitEventTimeout` as a REAL
...
docs/archive/0018 — quake mouse look); **system cursors** (`SDL_CreateSystemCursor` /
`SDL_SetCursor` / `SDL_GetCursor` / `SDL_GetDefaultCursor` /
`SDL_DestroyCursor` / `SDL_ShowCursor` / `SDL_HideCursor` /
`SDL_CursorVisible`; landed 2026-07-11, docs/archive/0105) — the pointer is the
...
**Policy decided 2026-07-07** (with the indefinite deferral of `docs/archive/0006`
...
### Clipboard (SDL_clipboard) — ✓ landed (docs/archive/0090, text)
...
- **No JSPI:** anything that "blocks" (Delay, device await, clipboard read)
  ...
  `WaitEventTimeout` really block on the OS input ring (docs/archive/0161), the same
