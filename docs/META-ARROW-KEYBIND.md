...
docs/KEYBINDING-OVERRIDE-SYSTEM.md). The BAKED decisions:
...
`docs/KEYMAP.md`'s "CLOSED DECISION" section before you propose anything that
...
**This deliberately revisits a recorded decision**: `docs/KEYMAP.md`
...
- **WM policy** — `os/wm.c:3726` routes the event to `snap_key()`
  ...
  snapped/maximized window or minimizes a floating one. All from docs/archive/0095.
- **Browser layer** — `os/os.html:630-638`: the VT2 canvas keydown handler
  ...
  Aero Snap works there today (`docs/KEYMAP.md` "⌘ … already reaches the WM
...
exists and shipped.** `os/keys.h` (docs/archive/0149+0150, design `docs/KEYMAP.md`)
...
1. **In the windows scheme, GUI+arrow line-nav is the wrong idiom anyway.**
   ...
   (`docs/KEYMAP.md` "gucOS is deliberately Win95/Win7, so Ctrl-style is
