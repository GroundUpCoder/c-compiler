# docs/LIABILITIES.md — the liability register (docs/archive/0286)
...
sweep found 12 such gaps. All 12 survived by one mechanism: **they never entered `docs/`.**

`node tools/liabilities/liabilities.js check` validates this file (also run by the `todos` suite in
...
and a legacy 4-digit `NNNN` resolves against `docs/archive/` (the archive). The check fails on:
...
exists to eliminate. `node tools/liabilities/liabilities.js list` resolves each anchor to a live
...
3. **Finding a pre-existing one.** The recurring liability sweep (`docs/archive/0302`) is the
...
- ticket: #130
- file: compiler.js
- anchor: // more than 65520 blocks still degrade to the linear chain — see docs/archive/0335.
...
- ticket: #145
- file: tools/liabilities/idspace.js
- anchor: // coordination point (a pushed reservation ref), which is docs/archive/0364, register
- provenance: 0360
...
- ticket: #168
- file: compiler.js
- anchor: inheritance through __spawn is a separate, ticketed gap (docs/archive/0399). */
- provenance: 0382

### L58 — no fd can refer to a directory, so the *at family's dirfd resolution mode is unreachable
- ticket: #169
- file: compiler.js
- anchor:  * docs/archive/0400 (directory file descriptors: O_DIRECTORY, dirfd(3), fdopendir(3))
- provenance: 0325

### L59 — mkfifo is absent on purpose; a link-testable stub would move a consumer's failure from configure time to run time
- ticket: #170
- file: compiler.js
- anchor:    time. Funded by docs/archive/0401, which is also what keeps docs/archive/0382 open. */
- provenance: 0382
...
### L72 — the gucOS http fetcher refuses multipart POST (loud FETCH_ERROR): the docs/archive/0433 file gadget builds a correct fetch_multipart_data list whose bytes still cannot reach a server
- ticket: #360
- file: vendor/netsurf/gucos/httpfetch.c
- anchor: 			"multipart POST is not supported yet (docs/archive/0433)");
- provenance: 0433
