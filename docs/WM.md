...
- **Status**: designed 2026-07-07 (docs/archive/0007); **v1 IMPLEMENTED the same
  day** (docs/archive/0012 spikes + docs/archive/0013 — see "Implementation status v1"
  below for what shipped and where it deviates); **/bin/wm + wmctl landed**
  (docs/archive/0014, status section below); **the acceptance test passed**
  (docs/archive/0015 — doom/snake/gameboy windowed in-OS with zero source
  changes; quake awaits the relative-mouse flag, docs/archive/0018); **GPU apps +
  the Dawn tier landed** (docs/archive/0016, 2026-07-08 — /bin/gpubox through
  ...
  **audio mixing landed** (docs/archive/0017, 2026-07-08 — the kernel sound
  ...
  **client resize landed** (docs/archive/0019, 2026-07-08 — SURFACE_CONFIGURE
  buffer renegotiation + kernel-chrome frame resize drags; status section
  below); **relative mouse / quake landed** (docs/archive/0018, 2026-07-08 —
  ...
  **resizable gating landed** (docs/archive/0021, 2026-07-08 —
  ...
  status — client resize"); **VT switching landed** (docs/archive/0022,
...
headlessly. **PASSED 2026-07-07** (docs/archive/0015) for doom, snake and
...
joined 2026-07-08** (docs/archive/0018 — the relative-mouse/pointer-lock
...
5. **Mechanism in the kernel, policy in the WM client** (a wasm binary
   speaking a protocol over AF_UNIX — dogfooding docs/archive/0008).
...
SURFACE_SET_FLAGS(id, flags)           flag-word update (docs/archive/0018; 0x1006):
                                       bit0 borderless, bit1 relative-mouse,
                                       bit2 resizable (docs/archive/0021)
SURFACE_CONFIGURE(id, w, h, serial)    the client's resize ACK (docs/archive/0019;
...
*(Status: LANDED — `docs/archive/0055`, 2026-07-09. os/compositor.js renders
...
(rasterized by the ksvc kernel text service since docs/archive/0275 — our
...
round-trips to the UI bridge for pointer lock *(landed — docs/archive/0018;
...
*(It did — docs/archive/0019; see "Implementation status — client resize"
below.)*

## The terminal (v2 LANDED, docs/archive/0020: /bin/term, the pure path)
...
item, docs/archive/0022). The old v1 idea of tracking xterm as a scene-positioned
...
- ~~**Audio mixing**~~ — designed + landed (docs/archive/0017); see "Audio mixing —
...
- ~~**wasm terminal app**~~ — landed with the pty layer (docs/archive/0020);
...
- ~~**Screen geometry / VTs / scaling fixed-size clients**~~ — designed
  ...
  LANDED (docs/archive/0022), dynamic screen resolution LANDED
  (docs/archive/0023), viewport scaling LANDED (docs/archive/0024),
  maximize LANDED (docs/archive/0025). The block is complete.
- **The desktop shell** — start menu, desktop icons, title-bar
  min/max boxes, taskbar polish, window cycling: designed below ("The
  desktop shell"), queued as docs/archive/0028–0033.
...
VT switching (docs/archive/0022), dynamic screen resolution
(docs/archive/0023), scaling fixed-size clients (docs/archive/0024),
maximize (docs/archive/0025 — nearly pure policy by then, dispatching on
...
**VT switching (docs/archive/0022 — LANDED 2026-07-08)** — the Linux
...
then a healthy `ready` auto-switches to VT2 (docs/archive/0070 — desktop is the
...
**Dynamic screen resolution (docs/archive/0023 — LANDED 2026-07-08)** —
...
**Maximize (docs/archive/0025 — LANDED 2026-07-08**; rides 0021's flag).
...
**Scaling fixed-size clients (docs/archive/0024 — LANDED 2026-07-08)**.
...
## The desktop shell (design, 2026-07-08 — queued: docs/archive/0028–0033)
...
- **WMP RESTACK place=1 already means "send to bottom"**, and borderless
  ...
  docs/archive/0038 the desktop pin is SET_LAYER -1 — a real pin, not a
  one-shot restack.)
...
**Start menu (docs/archive/0028).** A ~50px Start button at the taskbar's left
...
retired by docs/archive/0066: launching is one `activate()` — symlinks and
runnable files (wasm magic / `#!` scripts, docs/archive/0065) spawn, everything
...
**Start menu v2 — Win95 classic (docs/archive/0078, landed 2026-07-10).** The
...
**Start menu v3 — single Win95 column with a gucOS sidebar (docs/archive/0098
two-pane, reverted to one column by docs/archive/0132, gucOS band + bottom
...
(user decision 2026-07-12, options in docs/archive/0132), the **right pane is
...
<input>`; **Shut Down joins when docs/archive/0051 lands**), then — XP/Vista/7
...
docs/archive/0132 — dropped as too heavy for a fixed-height pane without
scrolling).

**Desktop icons (docs/archive/0029).** A fullscreen borderless wm surface,
pinned to the bottom z layer at create (SET_LAYER since docs/archive/0038;
...
*(since docs/archive/0066: the shared `activate()` — runnable files spawn too;
since docs/archive/0072 non-runnables open through the openwith associations —
...
*Selection & manipulation (docs/archive/0077, landed 2026-07-11).* All
...
docs/archive/0091/0101). wmctl grew `keydown`/`keyup` (one key edge — hold a
...
**Title-bar buttons (docs/archive/0030).** Today the bar has ONE box (close).
...
**Taskbar polish (docs/archive/0031).** A right-aligned clock (`time()` + the
...
**Taskbar polish round 2 (docs/archive/0101).** Right-clicking the strip
...
**Window cycling (docs/archive/0032).** The one shell piece needing new kernel
...
**Bug sweep (docs/archive/0033, repeatable format).** One session: drive the
...
`docs/archive/0064` (the pointer-lock HUMAN check, deferred by rounds 1 AND 2,
is its non-negotiable MUST).

## Known issues (standing list; round 1 = 2026-07-08 docs/archive/0033, round 2 = 2026-07-09 docs/archive/0039)

Verified-but-unfixed, each with a repro. Re-check every sweep; entries
graduate to queue items when a fix is scheduled.

- ~~**The taskbar is not always-on-top.**~~ **FIXED in docs/archive/0038**
...
- ~~**The focus fall landed on pinned furniture.**~~ **FOUND + FIXED in
  round 2 (docs/archive/0039)**: after 0038 the destroy/minimize focus fall
...
- **Pointer-lock UX needs a HUMAN check each round** (Chromium denies
  ...
  (`docs/archive/0064`, which numbers this so it cannot slip a third time).
...
- **A modifier held BEFORE the desktop ever holds focus is invisible to
  the first desktop click** (docs/archive/0077): pointer records carry no mod
...
- ~~**First-frame teleport.**~~ **FIXED in docs/archive/0069** (2026-07-10) by
...
## Implementation status — screensaver (landed 2026-07-11, docs/archive/0096)
...
## Implementation status — Aero Snap (landed 2026-07-11, docs/archive/0095)
...
## Implementation status — context menus (landed 2026-07-11, docs/archive/0091)
...
## Implementation status — window system menu (landed 2026-07-11, docs/archive/0102)
...
## Implementation status — Recycle Bin, desktop side (landed 2026-07-11, docs/archive/0093)
...
## Implementation status — Aero effects (landed 2026-07-10, docs/archive/0063)
...
## Implementation status — z layers (landed 2026-07-08, docs/archive/0038)
...
## Implementation status v1 (landed 2026-07-07, docs/archive/0013)
...
- **os/compositor.js** (kernel-worker side): ONE WebGPU render pass per
  rAF (docs/archive/0055; was Canvas2D in v1) — desktop clear, shm surfaces
  ...
  by the kernel's ksvc text service since docs/archive/0275 — kernel.textService,
...
- **os/os.html**: desktop canvas pane (natural size; 800×500 at boot,
  viewport-tracking on VT2 since docs/archive/0023) transferred to the
...
- ~~**Compositor is Canvas2D**~~ — CLOSED (landed 2026-07-09,
  `docs/archive/0055`): the WebGPU pass this doc designed is now the ONLY
...
- **Cursor is the native browser cursor** (no kernel sprite; headless
  ...
  host-borrow audit — docs/archive/0276, `logs/2026-07-22/host-borrow-audit.md`):
  ...
  Which native cursor shows IS ours to decide (docs/archive/0105): per-surface `SDL_SetCursor` shape
...
- ~~**Audio in OS processes stays null**~~ — lifted by docs/archive/0017 (the
  kernel sound server; see "Audio mixing" below).

## Implementation status — the WM client (landed 2026-07-07, docs/archive/0014)
...
## Implementation status — GPU apps + the Dawn tier (landed 2026-07-08, docs/archive/0016)
...
## Audio mixing — the kernel sound server (design + landed, docs/archive/0017)
...
the MASTER GAIN (docs/archive/0048: `AUDIO_GAIN` 0x2003 — percent 0..200,
...
leak one dead table entry per one-shot clip (docs/archive/0094's PlaySound
...
## Implementation status — client resize (landed 2026-07-08, docs/archive/0019)
...
- **Resizable gating (landed 2026-07-08, docs/archive/0021)**: SDL3 semantics —
...
## Implementation status — relative mouse / quake (landed 2026-07-08, docs/archive/0018)
...
|---|---|---|
| 1 (docs/archive/0012) | spikes S1–S5 | 1 |
