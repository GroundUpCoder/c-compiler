...
**Status**: SHIPPED (docs/archive/0338, image v179). Control-panel surface:
`docs/archive/0130` — its *command* half (the picker leg) landed with 0338; the
...
was since renamed `cpython-clang` by docs/archive/0374) fixes the **suggestion** shown
...
Three small, shared additions — all of them wanted by `docs/archive/0130` too:
...
## 8. The control panel — `docs/archive/0130`
