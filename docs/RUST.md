...
are `docs/archive/0413` … `docs/archive/0418`. This file holds what those units share: the ABI
...
- **Precedent**: `docs/CLANG-CPP-EPIC.md`. That document is the same problem, one
...
- **The gucOS Rust toolchain is funded unconditionally.** `docs/archive/0413` …
  `docs/archive/0416`. The path is bounded work. A probe already ran it end to end.
- **The application half is gated on measurement.** The first target application is
  ...
  half materially under-costed. `docs/archive/0418` is the decider that rules on the Rust
  ...
  **RULED 2026-07-30**: the standard library is upstream's, on `wasm32-wasip1`
  ...
  `docs/archive/0418-rust-std-decider.md` §"Result". C2 is filed as `docs/archive/0442`.
- **The plan of record is API-key-only, because `codex login` is out of scope.**
  ...
  **False**: `supports_websockets` IS a configuration key on a model provider
  ...
  ⇒ **A WebSocket transport is NOT a prerequisite for this program. `docs/archive/0417`
  is.** `docs/archive/0418` scope fact (a) carries the full correction, including which
...
- **`docs/archive/0417` (HTTP transfers become OFDs) is a hard, unconditional
...
contract is small, and it is fixed. `docs/archive/0413` writes it into a header comment in
...
does not. `docs/archive/0413` Trap 1 holds the detail.
...
miss fails at link time with the name of the symbol. `docs/archive/0413` Trap 2 holds the
detail.
...
### The wasip1 entry — the `docs/archive/0442` amendment
...
1. **One ABI — amended by the `docs/archive/0418` ruling (2026-07-30).** Rust reaches
   ...
   programs (`docs/archive/0442`). That namespace is frozen upstream, so it has no
...
2. **One libc.** `docs/archive/0414` (`gucos-sys`) is the single Rust-side declaration of
...
5. **The base image ships no Rust.** Rust binaries reach a user through opt-in
   ...
   `docs/archive/0416` carries a guardrail that proves the base image stays
   byte-identical.
6. **Loud failure, never a silent skip.** An explicit request for Rust with no
   ...
   sibling stays a normal state. This is rule 2 of `docs/CLANG-CPP-EPIC.md` §4,
   unchanged.
...
| `0447` | **D1** | codex on gucOS: port `codex exec`, or write a native client. **Ruled 2026-07-31** (ticket `#192`): `docs/RUST-D1-RULING.md`. |
...
`docs/RUST-D1-RULING.md`). The ruling is staged, and it does not pre-judge —
it keeps the `docs/archive/0418` §Result item 9 refusal. What it settles now: a
...
`docs/archive/0418` ruled (2026-07-30): the program takes the `wasm32-wasip1` path for
