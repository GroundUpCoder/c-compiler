...
`wmctl: wait ... timed out after Nms` in captured output — the docs/archive/0171 rule.
...
the entire headline repro of docs/archive/0274.
...
**Label:** `'manual VT choice during boot survives ready (docs/archive/0070)'`
...
1. **H2 + H3 (os-term menu open/dismiss).** Highest. Both drive
   ...
   focus is unreliable on VT2"), the menu bar is recent work (docs/archive/0273c on the
   0259 menucore engine — a component whose z-order/focus behaviour has already
   regressed once, docs/archive/0282), nothing else in the file covers it, and the two
