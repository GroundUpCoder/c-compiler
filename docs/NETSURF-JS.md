...
LANDED (2026-07-27, docs/archive/0289)**; lanes D–E open (E's scaffold shipped).
...
docs/archive/0316, and it retires §10.9's own diagnosis**).
...
**DONE 2026-07-27** (`docs/archive/0289`) — the design below landed as written;
...
9. **Still open, deliberately.**  `mouseover`/`mouseout`/`mouseenter`/
   `mouseleave` and `focusin`/`focusout` are **docs/archive/0317**: unlike
   ...
   measured; it was **docs/archive/0316**, P0, and it is why
...
## 11. What docs/archive/0316 found that §9 and §10 had wrong
...
## 12. What a page that re-boxes continuously shows (docs/archive/0407)
...
answer, because docs/archive/0407 named an alternative — a limit on the re-box rate
...
The defect was a different one.  Until docs/archive/0407 the half-built tree LEAKED
...
is the one docs/archive/0407 made.  The displayed frame must hold complete state
...
## 13. Which box a select gadget's text belongs to (docs/archive/0412)
...
`control->box`, and docs/archive/0412 had to decide which box it must write now that
...
**A related gap, found while deciding this, is filed as docs/archive/0422.**  In
...
option, and docs/archive/0412 made all of them correct, but no gucOS test can reach
them today.
