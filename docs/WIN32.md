...
clipboard (originally a file at `$HOME/.clipboard`; since docs/archive/0090 the
...
docs/archive/0135 — a real single-level undo record (the Win95 plain-EDIT
...
(ChooseFontW became REAL in docs/archive/0223: the file-dialog modal shape —
...
landed with docs/archive/0104 — see below),
...
wm.c desktop/taskbar menus are separate machinery (docs/WM.md — the
...
OS — docs/archive/0224 — and both block the same way now),
...
design `docs/SOFTWARE-NATIVE.md` §3) gave comctl32 its first REAL
...
narrowing → `docs/archive/0384`. Acceptance: `ctldemo listview` +
...
- **advapi32 hive**: writes since the last flush are lost on SIGKILL
  ...
  — was fixed by **docs/archive/0288**: `hive_flush` re-reads the hive and
  ...
  genuinely transactional store is still `docs/archive/0162`.
