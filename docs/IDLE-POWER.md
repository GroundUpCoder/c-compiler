...
**Owns**: the unifying design behind docs/archive/0160, docs/archive/0161, and the new
...
2. **Every app worker wakes ~60×/sec even when idle — but NOT the way the
   first draft said.** Review finding: **docs/archive/0100's vsyncWait pacing was
...
- **C. Idle apps stop producing frames** (docs/archive/0161). Seam verified:
...
2. **`test_wm_service_e2e`: 3 legs fail on clean main** (dblclick-on-term,
   ...
   omits the notepad icon added by `785eca2`; same class docs/archive/0164 fixed
...
- **Stage 2**: C1 (WaitEvent/WaitEventTimeout) + C2 (mgp). Real wakeup
  reduction on its own; no compositor risk. **DONE 2026-07-13 (docs/archive/0161).**
...
## Follow-on: the unified wait (docs/archive/0178, post-Stage-4)
...
regresses nothing. Scope, plan, acceptance: docs/archive/0178.

**LANDED 2026-07-14** (docs/archive/0178): kernel `FS_WAIT` (0x0420, FS_SELECT's
...
- Rework docs/archive/0160 (folds into A + D) and docs/archive/0161 (piece C) per this
