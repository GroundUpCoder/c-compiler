...
static int same_opened(SeditDocument*d){struct stat st;char real[sizeof d->target_path];
 ...
 if(io.stat_fn(real,&st))return 0;if(st.st_dev!=d->opened_stat.st_dev||st.st_ino!=d->opened_stat.st_ino)return 0;char h[65];return io.sha256_path_fn(real,h)==0&&!strcmp(h,d->opened_hash);}
