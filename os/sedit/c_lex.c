...
enum { L_NORMAL, L_BLOCK, L_LINE, L_STRING, L_CHAR, L_PREPROC,
       L_IDENT, L_NUMBER, L_SLASH, L_DOT };
...
static int lex_normal_run(SeditLexer*l,const char*s,size_t*i) {
...
int sedit_lex_finish(SeditLexer*l){if(l->failed)return 0;int k=0;if(l->state==L_IDENT){int ty=0;if(l->word_len<sizeof l->word&&kw(l->word,l->word_len,&ty)&&!tok(l,l->token_start,l->offset,ty?SEDIT_T_TYPE:SEDIT_T_KEYWORD))return 0;}else if(l->state==L_NUMBER)k=SEDIT_T_NUMBER;else if(l->state==L_BLOCK||l->state==L_LINE)k=SEDIT_T_COMMENT;else if(l->state==L_STRING)k=SEDIT_T_STRING;else if(l->state==L_CHAR)k=SEDIT_T_CHAR;else if(l->state==L_PREPROC)k=SEDIT_T_PREPROCESSOR;if(k&&!tok(l,l->token_start,l->offset,k))return 0;while(l->stack_count){uint32_t a=(uint32_t)(l->stack[--l->stack_count]>>8);if(!pair(l,a,a,1))return 0;}l->state=L_NORMAL;return 1;}
