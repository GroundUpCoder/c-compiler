...
/* One style entry per LINE RUN of [a,e): the batch bound must be counted in
 * the same unit it is written in (#728 — sizing per token overflowed the
 ...
 *truncated=0;
 ...
 for(size_t i=0;i<lex->token_count;i++)need+=line_runs(text,lex->tokens[i].start,lex->tokens[i].end);
 if(need>GUCEDIT_MAX_STYLES)need=GUCEDIT_MAX_STYLES; /* the consumer refuses larger batches; truncate and report */
 uint32_t cap=(uint32_t)need;
 GUCEDIT_BATCH_V1*b=malloc(sizeof(*b)+(size_t)cap*sizeof(GUCEDIT_STYLE_V1));if(!b)return NULL;uint32_t c=0;int complete=1;
 for(size_t i=0;i<lex->token_count;i++)complete&=append_style(b,cap,&c,text,lex->tokens[i].start,lex->tokens[i].end,color_for(lex->tokens[i].kind),0);
 uint32_t at=UINT32_MAX,mate=0;int unmatched=0;if(text){if(caret<len&&strchr("()[]{}",text[caret]))at=caret;else if(caret&&caret<=len&&strchr("()[]{}",text[caret-1]))at=caret-1;}
 if(at!=UINT32_MAX&&sedit_pair_mate(lex,at,&mate,&unmatched)){complete&=append_style(b,cap,&c,text,at,at+1,unmatched?SEDIT_RGB(180,0,0):SEDIT_RGB(0,70,160),GUES_BOX);if(!unmatched&&mate!=at)complete&=append_style(b,cap,&c,text,mate,mate+1,SEDIT_RGB(0,70,160),GUES_BOX);}
