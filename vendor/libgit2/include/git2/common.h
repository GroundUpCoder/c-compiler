...
/* ---- c-compiler link hint (ticket #632) ----
 * Every public git2 header reaches this file, and none of them carries a
 * __require_source block: libgit2 deliberately does NOT auto-link, unlike
 * <zlib.h>/<png.h>/<jpeglib.h> (#498). Two measured reasons (#632):
 *   - the TU set is 211 translation units (~19 s per in-OS compile), an
 *     order of magnitude past the largest auto-required library, and
 *   - libgit2 links its own vendored deps/zlib, so an auto-required
 *     <git2.h> would make ANY program that also includes <zlib.h> or
 *     <png.h> fail with duplicate definitions (adler32, crc32, ...) even
 *     when it never calls a git_* function.
 * Linking is one explicit opt-in — `#include <git2_srclib.h>` after the
 * git2 headers (generated by tools/mkgit2srclib.js, shipped by the libgit2
 * srclib package). The hint below keeps the default include path honest:
 * an undefined git_* symbol at link time names that fix instead of dying
 * as a bare "Undefined symbol". */
__link_hint("git_", "libgit2 does not link automatically: add '#include <git2_srclib.h>' after the git2 headers (shipped by the 'libgit2' srclib package)");
