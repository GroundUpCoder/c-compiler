...
- **Shipped as**: the `micropython` gucman package (`packages/micropython.json`)
  ...
  name `python` (docs/archive/0338): installing appends `python<TAB>/usr/local/bin/
...
MicroPython), and gucOS already owns mounting in the kernel (`docs/KERNEL.md`)
...
The feature set is the `docs/archive/0117` **Round 2** target: a real script runner
...
* The writable site dir is under **`/usr/local`**, not `/usr/lib`: `/usr` is a
  sealed read-only volume (`docs/archive/0040`), so a site dir there could never be
...
Before `docs/archive/0117` R1 these headers were hand-extended, which is why
...
- **`python` is a MicroPython dialect, not CPython.** No `pip`, no C-extension
  ...
  `docs/archive/0117`'s "Why". gucOS now has a second, independent python route as
  well (`docs/CPYTHON.md`); which one the bare `python` command runs is the
  dispatcher's business, `docs/archive/0338`.
- **Modules a Python programmer will reach for and not find**: `datetime`,
  ...
  that does not compose with the kernel's unified WAIT (`docs/archive/0178`) would be
  ...
  All of it is `docs/archive/0117` R3, demand-driven.
...
- **`os.sync()` is absent**, deliberately: upstream's body only syncs FatFS
  ...
  file object's `.flush()`, which is a real kernel `FS_FSYNC` (`docs/archive/0036`).
