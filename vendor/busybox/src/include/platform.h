...
/* WASM PORT PATCH: the kbuild passes autoconf.h via `-include`; this
 * compiler has no such flag, and platform.h is the first header every
 * busybox TU pulls (via libbb.h) — so include the config here. */
#include "autoconf.h"
...
#if defined(__wasm__)
/* WASM PORT PATCH: what this libc doesn't carry; libbb/platform.c supplies
 * the fallbacks platform.h declares below. (dprintf, strchrnul, clearenv,
 * fdatasync, usleep, sighandler_t & the ext-lib fnmatch DO exist here.) */
# undef HAVE_MEMPCPY
# undef HAVE_STPCPY
# undef HAVE_STPNCPY
# undef HAVE_STRSEP
