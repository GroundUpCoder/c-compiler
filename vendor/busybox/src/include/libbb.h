...
/* WASM PORT PATCH: no inline asm; wasm has a single thread and this
 * compiler doesn't reorder across calls — the barrier can be empty. */
#ifdef __wasm__
...
/* WASM PORT PATCH: this compiler has no GNU statement expressions; the
 * three ({...}) macros become ALWAYS_INLINE helpers like isalnum below
 * (same single-evaluation guarantee). */
...
/* WASM PORT PATCH: the port layer (glibc-isms + the vfork-on-__spawn
 * shim's journaling intercepts) — included last so it sees every
 * declaration above. See vendor/busybox/port/. */
#include "wasm_port.h"
