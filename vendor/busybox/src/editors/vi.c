...
#endif
	int c;
	/* WASM PORT PATCH: "int sig" gone — sigsetjmp moved into the if () below
	 * (this compiler's setjmp lowering only supports the if-form). */
...
//	Ureg = 27; - const		// hold orig line for "U" cmd
	...
	signal(SIGWINCH, winch_handler);
	signal(SIGTSTP, tstp_handler);
	/* WASM PORT PATCH: "sig = sigsetjmp(...); if (sig != 0)" → supported
	 * if-form (the sig value was only ever tested against 0). */
	if (sigsetjmp(restart, 1) != 0) {
