...
#include <glob.h>
/* #include <dmalloc.h> */
/* WASM PORT PATCH: ENABLE_HUSH_CASE isn't defined yet at this point (the
 * kbuild -include's autoconf.h before everything; our port pulls it in
 * via busybox.h below), so the guard would silently drop the include.
 * Unconditional is harmless. */
#include <fnmatch.h>
#include <sys/times.h>
...
# endif
	...
	xpipe(channel);
	/* WASM PORT PATCH: vfork -> the journaling __spawn shim (port/).
	 * The "child" block runs in-process: fd ops journal into the spawn
	 * spec; re_execute_shell's execve issues the spawn and longjmps back
	 * here with the real pid (_exit -> synthetic pid). */
	if (setjmp(pv_state.jmp) == 0) { pv_child_begin(); /* "child" */
...
# endif
	...
	/* parent */
	pid = pv_state.child_pid;  /* WASM PORT: set by the shim's longjmp */
	*pid_p = pid;
# if ENABLE_HUSH_FAST
	G.count_SIGCHLD++;
...
static void setup_heredoc(struct redir_struct *redir)
{
	...
	/* WASM PORT PATCH: no fork, so the "orphaned grandchild keeps writing
	 * into the pipe" trick can't work — and a spawned writer would
	 * deadlock (the consumer execs only after we return). Do what bash
	 * does instead: the heredoc body goes through an unlinked temp file.
	 * (The pipe fast path below is skipped: pair would need a writer.) */
	{
		char pv_tmpname[64];
		int pv_tfd;
		sprintf(pv_tmpname, "/tmp/.hush-heredoc-%d-%p", (int)getpid(), (void*)redir);
		pv_tfd = open(pv_tmpname, O_WRONLY | O_CREAT | O_TRUNC, 0600);
		if (pv_tfd < 0 && errno == ENOENT) {
			mkdir("/tmp", 0777);       /* image without /tmp: self-heal */
			pv_tfd = open(pv_tmpname, O_WRONLY | O_CREAT | O_TRUNC, 0600);
		}
		if (pv_tfd < 0) {
			bb_perror_msg("can't create heredoc file");
			free(expanded);
			return;
		}
		if (len)
			full_write(pv_tfd, heredoc, len);
		close(pv_tfd);
		xmove_fd(xopen(pv_tmpname, O_RDONLY), redir->rd_fd);
		unlink(pv_tmpname);
		free(expanded);
		return;
	}
#if 0  /* WASM PORT: original pipe + double-vfork feeder, disabled */
	xpiped_pair(pair);
...
# endif
	 ...
	 * non-builtin later. */
	/* WASM PORT PATCH: upstream NOMMU uses find_builtin1 (special
	 * builtins only) because regular ones are cheaper as re-exec'ed
	 * APPLETS of the multicall binary — which doesn't exist here. Route
	 * ALL builtins through exec_builtin -> re_execute_shell -> __spawn
	 * of /bin/sh instead (same cost as any spawn on this platform). */
	x = find_builtin(argv[0]);
	if (x) {
...
//therefore CLOEXEC on saved fd does not help.
 ...
 must_fork:
	...
	while (cmd_no < pi->num_cmds) {
		...
		/* WASM PORT PATCH: vfork -> the journaling __spawn shim (port/);
		 * pseudo_exec's exec (or _exit) longjmps back with the pid. */
		if (setjmp(pv_state.jmp) == 0) { pv_child_begin(); /* "child" */
#if ENABLE_HUSH_JOB
			disable_restore_tty_pgrp_on_exit();
			CLEAR_RANDOM_T(&G.random_gen); /* or else $RANDOM repeats in child */
			...
			if (pi->alive_cmds == 0 && pi->followup == PIPE_BG) {
				...
				/* WASM PORT: journal "stdin <- /dev/null" explicitly
				 * (the original's open()-lands-on-fd-0 trick assumes
				 * the close(0) really happened). */
				int pv_nullfd = open(bb_dev_null, O_RDONLY);
				if (pv_nullfd >= 0) {
					dup2(pv_nullfd, 0);
					close(pv_nullfd);
				}
		...
		/* parent or error */
		command->pid = pv_state.child_pid;  /* WASM PORT: from the shim */
#if ENABLE_HUSH_FAST
		G.count_SIGCHLD++;
...
int hush_main(int argc, char **argv)
{
	 ...
	 * passed separately; the re-exec always execve's /bin/sh, so argv[0] here
	 * only feeds login detection. */
