...
#ifndef HAVE_WAIT3
/* WASM PORT PATCH: this libc has no wait3/wait4 — waitpid + getrusage.
 * getrusage() here zeroes every field (child CPU time is not tracked on
 * this platform, the same rule as /proc's 0 cpu-time fields), so the
 * user/sys figures print 0.00 while the elapsed (wall) fields stay real
 * (monotonic_ms). */
static pid_t wait3(int *status, int options, struct rusage *rusage)
{
	pid_t p = waitpid(-1, status, options);
...
static void run_command(char *const *cmd, resource_t *resp)
{
	...
	/* WASM PORT PATCH: vfork site -> the setjmp journaling-shim form
	 * (wasm_port.h; same rewrite as hush's three sites and tar's
	 * vfork_compressor). The "child" runs in the parent journaling its
	 * ops; BB_EXECVP_or_die's execvp issues the one __spawn and longjmps
	 * back with the real pid. */
	if (setjmp(pv_state.jmp) == 0) {
		pv_child_begin();
		/* Child */
		BB_EXECVP_or_die((char**)cmd);
	}
	pid = pv_state.child_pid;
