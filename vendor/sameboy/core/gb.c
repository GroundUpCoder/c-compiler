...
#include "random.h"
#include "gb.h"
#include <alloca.h> /* PATCH(c-compiler): alloca is not in <stdlib.h> in this libc */


void GB_attributed_logv(GB_gameboy_t *gb, GB_log_attributes_t attributes, const char *fmt, va_list args)
{
    int errno_backup = errno;
    /* PATCH(c-compiler): no vasprintf in this libc — fixed-size buffer */
    char string[512];
    if (vsnprintf(string, sizeof(string), fmt, args) >= 0) {
        if (gb->log_callback) {
...
static void GB_reset_internal(GB_gameboy_t *gb, bool quick)
{
    ...
    memcpy(rtc_section, GB_GET_SECTION(gb, rtc), rtc_size);
    memset(gb, 0, GB_SECTION_OFFSET(unsaved));
    memcpy(GB_GET_SECTION(gb, rtc), rtc_section, rtc_size);
    gb->model = model;
