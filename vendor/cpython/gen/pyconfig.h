...
/* M0 probe: compiler.js declares __STDC_NO_ATOMICS__; the shim <stdatomic.h>
   in shim/ supplies a single-threaded lowering, so route pyatomic.h to its
   C11 branch. */
#undef __STDC_NO_ATOMICS__
#define HAVE_STD_ATOMIC 1

/* M0 probe: compiler.js declares clockid_t/struct timespec in <time.h> only,
   while CPython (like musl/wasi-libc) expects them visible via <sys/types.h>
   before Include/cpython/pthread_stubs.h is reached. Pull them in early. */
#include <sys/types.h>
#include <time.h>

/* M0 probe: compiler.js supports the _Thread_local storage-class specifier but
   also predefines __STDC_NO_THREADS__ (which properly describes <threads.h>, not
   the specifier), so Include/pyport.h concludes there is no TLS. Steer it. */
...
/* todos/0340: HAVE_SIGSET_T is absent from this configure's output entirely
   ...
   subprocess.run() call. The type is real here (libc: sigset_t is a 64-bit
   word) and posix_spawnattr_set{sigdefault,sigmask} are implemented, so the
   honest value is 1. */
