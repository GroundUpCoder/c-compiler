...
/* gucOS vendor patch (docs/archive/0340, CPYTHON.md §4.2): free_string_array /
   ...
   whose process model is owner-brokered posix_spawn with no exec at all — then
   compiles posix_spawn against three functions that were preprocessed away.
   Adding HAVE_POSIX_SPAWN to the guards is upstream-shaped: they are helpers of
   every caller in the block, and posix_spawn is one. */
