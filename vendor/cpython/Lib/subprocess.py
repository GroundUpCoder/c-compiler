...
if _mswindows:
    ...
    if _can_fork_exec:
        ...
        # _fork_exec is None on such a platform and the _execute_child fast path
        # below is the only route, which it guards for explicitly.
        try:
...
_USE_VFORK = True
# Upstream sends a bare program name ("ls") down the fork_exec path because
# os.posix_spawn does not search PATH. os.posix_spawnp does, and on a platform
# with no fork the fallback does not exist, so use it and keep the fast path.
_POSIX_SPAWN_SEARCHES_PATH = (sys.platform == 'gucos'
                              and hasattr(os, 'posix_spawnp'))
...
class Popen:
    ...
    if _mswindows:
        ...
        def _posix_spawn(self, args, executable, env, restore_signals, close_fds,
            ...
            if _POSIX_SPAWN_SEARCHES_PATH and not os.path.dirname(executable):
                spawn = os.posix_spawnp
            self.pid = spawn(executable, args, env, **kwargs)
            self._child_created = True
        ...
        def _execute_child(self, args, executable, preexec_fn, close_fds,
            ...
            if _fork_exec is None:
                # No fork() on this platform and the call did not qualify for
                # the posix_spawn fast path above. Say which knob disqualified
                # it — the alternative is a TypeError deep in the fork branch
                # naming nothing.
                raise OSError(
                    errno.ENOTSUP,
                    f"{sys.platform} has no fork(): subprocess supports only "
                    f"the posix_spawn path, which excludes preexec_fn, cwd, "
                    f"pass_fds, start_new_session, process_group, user/group "
                    f"and umask")
