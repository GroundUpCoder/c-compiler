...
`%`-directives (`SYNTAX` documents the format). Seeded as `/bin/mgp` with a
demo deck at `/usr/share/mgp/demo.mgp` (Start menu ▸ Demos ▸ mgp); `.mgp`
files open with it via the openwith table.
