...
| Window manager | **v1 LIVE** (todos/0007 design + 0012/0013 implementation, 2026-07-07): kernel surfaces (shm + bitmap transports), input rings, kernel-chrome policy, Canvas2D compositor in the kernel worker, agent channel with headless screenshots; `winbox &` opens a real window (`tests/browser/os-wm.mjs`). Next: /bin/wm policy client (todos/0014). Details: `todos/WM.md` "Implementation status v1". |
