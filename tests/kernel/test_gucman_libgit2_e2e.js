...
const PROG = [
  ...
  '    int reopen = (argc > 1 && strcmp(argv[1], "reopen") == 0);',
  ...
  '',
  '    if (reopen) {',
  '        if (git_repository_open(&repo, "/root/demo") < 0) return fail("open");',
  '    } else {',
  '        if (git_repository_init(&repo, "/root/demo", 0) < 0) return fail("repo_init");',
  '        git_config *cfg = NULL;',
  ...
  '        git_config_free(cfg);',
  '',
  '        FILE *f = fopen("/root/demo/hello.txt", "w");',
...
async function main() {
  ...
  const scriptB = [
    ...
    'cd /root && ./gitdemo reopen; echo RC=$?',
    'echo ==remove',
    ...
    'cd /root && cc gitdemo.c -o gitdemo2 2>&1 | head -3; echo CCRC=$?',
    'echo ==done',
    'exit',
  ];
