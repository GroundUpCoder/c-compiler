#!/usr/bin/env node
'use strict';
// Runs the kernel (process control plane) test suite. See docs/KERNEL.md.
//
...
//
// Engine: tests/lib/suite-runner.js (docs/archive/0081). Every file is independent
...
// Cross-tree preflight (docs/archive/0341) — FIRST, ahead of acquireHeavyLock(): a
...
// copying the prebaked os/os-system.img fixture (docs/archive/0082) — the runner
...
const tests = [
  ...
  ['test_hush_errexit_e2e.js', IMG], // #769: shell errexit loop-condition exceptions
  ...
  ['test_module_cache.js', BOOT], // 0037+#188: compiled-Module cache on spawn — RO immutable + rw VALIDATED keys, replace-on-rewrite, custom imports/no-fs exclusion, real clone e2e, in-OS recompile loop
  ...
  ['test_wm_frames.js', LIGHT], // #790 configure serials + buffer generations: issue/ack identity, ESTALE on retired/unknown/backward serials, the outstanding bound, equal-size regeneration, decline (EV_CONFIGURE_DECLINED, no pending leak), destroy with a configure in flight, gpu frame rejection by serial/size, pointer-record geometry epochs, GET_STATE/wmList identities, a seeded storm pinning SAB W/H/GEN == surface w/h/committedSerial after every op, SH_LOCK bounded-spin miss accounting (no wasm)
  ['test_shm_ownership.js', LIGHT], // #790 REAL frame ownership: a worker_threads producer hammering locked mailbox flips vs the kernel's front-buffer copy — zero torn screenshots; RED CONTROL: the same producer flipping WITHOUT SH_LOCK reproduces tearing (proves the instrument sees it)
  ...
  ['test_null_trap_e2e.js', BOOT, IMG, { timeoutMs: 600000 }], // #709: in-OS cc opt-in null trap -> named crash marker + shell status 139; default-off compile remains accepted
  ['test_abort_backtrace_e2e.js', IMG], // #760: real SIGABRT reaping preserves redirected diagnostics
  ['test_frame_lifecycle_e2e.js', IMG], // #764: callback fault settles, reaps, removes window; explicit exit retains status
  ['test_render_vsync_e2e.js', BOOT, IMG, { timeoutMs: 600000 }], // #500: SDL_SetRenderVSync/GetRenderVSync — vsyncWaitUntil park discipline (ARMED/doorbell, STOP releases the compositor, signal-EINTR, wrap), contract legs through the real veneer (default 0, round-trip, adaptive refused UNCHANGED, no-clock refusal), pacing in TICKS (vsync=1 one frame/tick, vsync=2 the divisor, multi-window), 60/90/120/144 Hz sweep
  ...
  ['test_gamepad_e2e.js', BOOT, IMG], // #607: SDL gamepad subsystem — pre-boot pad + focus reconcile (ADDED + held-state replay), PAD_NAME RPC name, parked-WaitEvent wake on pad button, gbutton/gaxis payloads + state getters, kernel dedup, live hotplug, REMOVED with dead-but-valid open handle + surviving name; then the real OS: baked /bin/padbox driven by wmctl pad connect/press/axis/disconnect over the WMP PAD_* ops
  ['test_sdl_events_e2e.js', BOOT], // #604: event-queue manipulation over the veneer queue — PushEvent/RegisterEvents round-trip (zero-timestamp stamping, caller timestamp preserved), HasEvent(s)/FlushEvent(s)/PeepEvents ADD/PEEK/GET incl. the NULL-events counting peek, a pushed event completing WaitEventTimeout without parking (the single-threaded wake contract), and PumpEvents advancing the #493 keyboard snapshot on an injected key with the KEY_DOWN left queued
  ['test_wgpu_loopguard_headless_e2e.js', BOOT, IMG], // #712 counter-pass: the headless refusal covers the raw-webgpu.h transport — second main-live wgpuSurfacePresent refuses exit 69 with the WebGPU-specific diagnostic; one present stays legal. Pins the guard's SYNCHRONOUS placement in __wgpu_surface_present (the async readback tail runs post-unwind where mainLive is false by construction — a guard there is inert, the first landing's defect). Deviceless on purpose (a blocking main can never acquire a Dawn device headless), so no webgpu-package skip
  ['test_loopguard_headless_e2e.js', BOOT, IMG], // #712: the #551 blocking-loop + GPU-present refusal fires HEADLESS too — naive default-tier present loop exits 69 with the headless-variant message at the second present; the SAME binary under SDL_RENDER_DRIVER=software runs to completion (the sanctioned escape, both hosts); one present from main() stays legal (the SDL_AppInit allowance). Browser twin: os-loopguard.mjs
  ...
  ['test_sleep_overshoot_e2e.js', BOOT], // #492: timed-sleep quality ladder — the OS timed-wait leeway (min(request/2,10ms) on macOS) stays compensated in all three substrates (usleep/nanosleep via KernelClient.park, pre-window SDL_Delay via blockingSleepMs, windowed SDL_Delay via pumpWait): per verb 40x16ms, p50 overshoot < 4ms (uncompensated measures 8ms) + the never-early contract bound; BOOT: measured 213MB peak RSS 2026-08-15
  ...
  ['test_netsurf_console_e2e.js', BOOT, IMG], // docs/archive/0421: a page's console output reaches the shell's tty — every console level under its own name, a multi-line entry prefixed on both lines, the trailing-newline and empty-entry branches, and `2>FILE` proven to be the off-switch
  ...
  ['test_cc_lua_sqlite_e2e.js', { timeoutMs: 900000 }], // #663: Lua + SQLite as linkable srclib packages — <lua.h> embeds the interpreter (fib via luaL_dostring, the LUA_USE_C89 32-bit lua_Integer ABI pinned) and <sqlite3.h> links the whole amalgamation (file-backed rows survive a reopen) from bare includes, each with its -D<LIB>_NO_REQUIRE_SOURCES hatch as the red control and the shipped shells asserted unchanged; the FIRST bin+srclib packages (the (a) ruling: one name, one owner — no -dev split), so the minimal image proves ONE `gucman install` plants bin+include+src together, `gucman remove lua` unplants all three, and a reinstall restores them; shares the cached minimal blob + mkpkg pool with the gucman e2es
  ...
  ['test_sdl_util_e2e.js', BOOT, { timeoutMs: 600000 }], // #601: the SDL3 trivial-absence batch via in-OS cc — SDL_malloc/calloc/realloc contracts over the SDL_free heap, deterministic re-seeded SDL_rand/srand (+rand_bits/randf) in-range, SDL_Log landing on stderr ONLY (fd-2 redirect split), GetBasePath (incl. the argv[0] symlink chase) + GetPrefPath really creating dirs, GetWindowFlags = exactly the create flags, GetRenderDrawColor default-white + round-trip, and the four batch draws (FillRects/Rects/Points/Lines-as-polyline) pixel-exact in a wmctl shot
  ...
  ['test_sdl_raster_e2e.js', BOOT, { timeoutMs: 600000 }], // #668: the software tier's triangle rasterizer — RenderLine diagonals are 1px LINES not bbox fills (the pre-#668 defect), RenderGeometry really draws (solid, indexed+textured NEAREST, Gouraud interpolation, renderer-BLEND arm), all pixel-asserted from a wmctl shot
  ['test_sdl_rendertarget_e2e.js', { timeoutMs: 600000 }], // #496: SDL_SetRenderTarget/SDL_GetRenderTarget on the software tier — TARGET-access gate (STATIC refused, bogus access refused at create), round-trip both ways, present-while-bound fails, content persists across re-bind, real target alpha (transparent region composites as nothing), bound-target-as-source refused; wmctl shot discriminates a target that silently draws to the screen (GPU twin: os-rendertarget.mjs)
  ['test_sdl_debugtext_e2e.js', { timeoutMs: 600000 }], // #494: SDL_RenderDebugText/DebugTextFormat on the software tier — upstream's built-in 8x8 font as a lazy per-renderer atlas tinted by the current draw color: glyph pixels exact from the embedded font bitmaps (H/I counterforms, 8px pen advance, format '%d', space advances without ink), half-alpha white lands exactly (128,128,255), U+00F7 pins upstream's >=190 invalid-checkerboard quirk verbatim, text into a #496 render target composites where the target lands, one present = one kernel frame (GPU twin: os-debugtext.mjs)
  ['test_sdl_rotated_e2e.js', { timeoutMs: 600000 }], // #672: SDL_RenderTextureRotated full contract — clockwise angle about the (NULL = middle) center, explicit non-default center as the real pivot (relocated footprint + untouched original), H/V/H|V flip bitmask, NULL src/dst rects, validation discipline; pixel-asserted from a wmctl shot on the #668 software rasterizer
  ['test_sdlttf_e2e.js', { timeoutMs: 900000 }], // ticket #468: the SDL_ttf CLASSIC API as a builtin veneer over FreeType (the SDL3_image contract) — fat image `cc ttfdemo.c` (no -I, no TU list) builds a program whose self-checking pixel asserts prove real ink: Blended AA + exact fg color + baseline-sane bbox, Solid bilevel, Shaded opaque bg box, dims == TTF_GetStringSize (one layout core), 2-byte UTF-8 e-acute as ONE glyph with a higher accent than 'e' (the not-Latin-1 control), invalid bytes render predictably, wrapping/styles (bold=more ink, underline=a real rule row)/hinting/kerning/metrics/measure round-trips, zero-width + null-font named errors, the fg.a==0-opaque quirk, and TTF_GetError == SDL_GetError; the missing-freetype loud failure + pay-for-what-you-use control live host-side in test_sdlttf_link.js
...
// at once is the docs/archive/0388 race that retargeted a sibling's repo mid-read.
