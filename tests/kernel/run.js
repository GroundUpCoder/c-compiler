...
const tests = [
  ...
  ['test_rust_e2e.js', IMG], // todos/0413: a no_std Rust binary runs in gucOS — committed fixture (sha256-pinned) spawned from the shell in a booted OS + the panic→__exit path (exit 101, no hang) + module shape per RUST.md §2; sibling-gated legs (gucos-rust at RUST_ROOT, default ~/git/gucos-rust): freshness rebuild must be byte-equal, missing #[link(wasm_import_module="c")] must FAIL AT LINK naming the symbol, a no-alloca module must trap at start-up; absent sibling SKIPs those legs unless RUST_REQUIRE=1 (then loud fail + fix command)
