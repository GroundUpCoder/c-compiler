...
// existing ConstEval: 1 when the operand is an integer constant
// expression, 0 otherwise (conservative — gcc docs permit returning 0
// for any argument). The operand is NOT evaluated (x++ must not run).
// The result is itself an integer constant expression (usable in a
// static array size). #587.
