/* select() with empty fd sets must honour its timeout under block-FS
 * (the "select as sleep" case — same Atomics.wait timeout as usleep). */
#include <stdio.h>
...
int main(void) {
    ...
    printf("ret: %d\n", ret);
    /* One-sided ON PURPOSE (docs/archive/0361). The lower bound IS the property
