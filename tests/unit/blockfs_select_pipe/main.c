/* select() readiness on a pipe under block-FS: an empty pipe times out;
 * a pipe with buffered data is readable immediately. */
#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    /* Select on read end of empty pipe — should time out. */
    FD_ZERO(&rfds);
    FD_SET(fds[0], &rfds);
    tv.tv_sec = 0;
    tv.tv_usec = 10000;
