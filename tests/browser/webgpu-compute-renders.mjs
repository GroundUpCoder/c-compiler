...
// webgpu-compute.c -> .html, drives it with Chrome (system channel, which
...
const PORT = 3201;
const URL = `http://localhost:${PORT}/webgpu-compute.html`;

const out = path.join(__dirname, 'www', 'webgpu-compute.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-compute] compiling webgpu-compute.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-compute.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-compute] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-compute] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-compute.png') });

  const { center } = s;
  // The program paints the WHOLE surface GREEN iff the GPU compute result
  // (data[i] == 2*i for all 64 elements) verified, RED on any mismatch. So a
  // green surface proves the compute pipeline ran and produced correct values.
  if (!(center.g > 200 && center.r < 60)) {
    throw new Error(`compute did not verify green (got ${JSON.stringify(center)} — red=mismatch, black=pending)`);
  }
  console.log('[webgpu-compute] PASS — compute doubled the array on the GPU; result verified (surface green)');
} catch (e) {
  console.error('[webgpu-compute] FAILED:', e.message);
