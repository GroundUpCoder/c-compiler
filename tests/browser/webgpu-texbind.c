/* WebGPU texture-binding viewDimension (Phase A9). A 2-layer array texture
   (layer0 GREEN, layer1 PINK) is bound through a bind-group layout whose texture
   entry declares viewDimension = 2D-array, and sampled in WGSL as
   texture_2d_array selecting layer 1. If the host still hardcodes the binding
   layout to '2d' (the old behavior), creating the bind group with a 2D-array
   view fails validation and nothing renders. A pink center proves the binding
   layout's viewDimension is honored. Driven by webgpu-texbind-renders.mjs. */
#include <webgpu.h>
#include <stddef.h>
#include <stdio.h>
...
static const unsigned char pink[4]  = { 255, 51, 204, 255 };
...
"  return textureSample(tex, samp, uv, 1);\n"   /* sample array layer 1 = PINK */
"}\n";
...
static void write_layer(WGPUTexture t, int layer, const unsigned char *rgba) {
    WGPUTexelCopyTextureInfo dst;
    zero(&dst, sizeof(dst));
    dst.texture = t;
    dst.origin.z = (unsigned)layer;
    dst.aspect = WGPUTextureAspect_All;
    WGPUTexelCopyBufferLayout layout;
    zero(&layout, sizeof(layout));
    layout.bytesPerRow = 4;
    layout.rowsPerImage = 1;
    WGPUExtent3D ext; ext.width = 1; ext.height = 1; ext.depthOrArrayLayers = 1;
    wgpuQueueWriteTexture(queue, &dst, rgba, 4, &layout, &ext);
}

static WGPUTexture make_array_texture(void) {
    WGPUTextureDescriptor td;
    zero(&td, sizeof(td));
    td.usage = WGPUTextureUsage_TextureBinding | WGPUTextureUsage_CopyDst;
    td.dimension = WGPUTextureDimension_2D;
    td.size.width = 1; td.size.height = 1; td.size.depthOrArrayLayers = 2;
    td.format = WGPUTextureFormat_RGBA8Unorm;
    td.mipLevelCount = 1; td.sampleCount = 1;
    WGPUTexture t = wgpuDeviceCreateTexture(device, &td);
    write_layer(t, 0, green);
    write_layer(t, 1, pink);
    return t;
}

static WGPUTextureView make_array_view(WGPUTexture t) {
    WGPUTextureViewDescriptor vd;
    zero(&vd, sizeof(vd));
    vd.dimension = WGPUTextureViewDimension_2DArray;
    vd.baseArrayLayer = 0;
    vd.arrayLayerCount = 2;
    vd.mipLevelCount = 1;
...
static WGPUBindGroupLayout make_bgl(void) {
    ...
    e[1].texture.viewDimension = WGPUTextureViewDimension_2DArray;   /* feature under test */
...
static void build(void) {
    ...
    WGPUTexture tex = make_array_texture();
    WGPUTextureView texView = make_array_view(tex);
...
int main(void) {
    ...
    printf("webgpu texbind: main done, waiting for adapter\n");
    return 0;
}
