...
import fixture from '../objc/os-script.js';
...
try {
  ...
  await page.keyboard.type('pbpaste > /root/objc-round2.sh && sh /root/objc-round2.sh; echo OBJC-RESULT=$?\r');
  ...
  check('Objective-C /bin/cc + browser process-worker exit 0', /OBJC-RESULT=0/.test(out) && out.includes('OBJC-COUNT='+fixture.count), out.slice(-1500));
} catch (e) { s.fail(e); }
