...
// webgpu-readback.c -> .html, drives it with Chrome (system channel, which
...
const PORT = 3199;
const URL = `http://localhost:${PORT}/webgpu-readback.html`;

const out = path.join(__dirname, 'www', 'webgpu-readback.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-readback] compiling webgpu-readback.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-readback.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-readback] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-readback] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-readback.png') });

  const { center, corner } = s;
  // This program reads the rendered offscreen pixel BACK from the GPU and paints
  // the WHOLE surface with it, so a correct readback makes every sampled pixel
  // EXACTLY the rendered pink (255,51,204) — center and corner alike. An exact
  // match proves copyTextureToBuffer + mapAsync + getMappedRange round-tripped
  // the right bytes (a broken readback would leave the surface black/garbage).
  const isPink = (p) => p.r === 255 && p.g === 51 && p.b === 204;
  if (!isPink(center)) throw new Error(`center not the read-back pink: ${JSON.stringify(center)}`);
  if (!isPink(corner)) throw new Error(`corner not the read-back pink: ${JSON.stringify(corner)}`);
  console.log('[webgpu-readback] PASS — GPU→CPU readback round-trip (surface == read-back pixel 255,51,204)');
} catch (e) {
  console.error('[webgpu-readback] FAILED:', e.message);
