...
try {
  ...
  // Read the dragged geometry via the shell. FLAGS has nine slots; the final
  // dash requires requested-hidden (H) clear, alongside the existing flag checks.
  await setVt(1);
  ...
  await setVt(2);
  // "Restored" tolerates the 0029 desktop-icon pixels — since the
  // 0184/0185 seeds this region holds parts of TWO icon columns (tiles +
  // labels, ~5-8% of it) — only the WINDOW must be gone (quake live is
  // ~90% non-teal here).
  await waitFrame([16, 40, 328, 232], s => s.nonTeal < s.n * 0.12, 30000);
...
console.log(state.failures === 0 ? '\nos quake (browser): PASS' : `\nos quake (browser): ${state.failures} FAILED`);
