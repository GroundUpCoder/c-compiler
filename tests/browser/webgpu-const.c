/* WebGPU pipeline-overridable constants (A12, WGPUConstantEntry). The shader's
   geometry coverage AND its color are decided by `override` constants whose
   defaults are the WRONG answer:
     - vertex override COVER defaults to 0.0 (degenerate triangle, no coverage);
       set to 1.0 -> the triangle covers the screen.
     - fragment overrides uR/uG default to red (1,0); set to (0,1) -> green.
   So a GREEN center proves BOTH the vertex and fragment stages honored their
   constantCount/constants (ignored constants would leave the center on the
   dark-blue clear, or red). Driven by webgpu-const-renders.mjs. */
#include <webgpu.h>
#include <stddef.h>
#include <stdio.h>
...
"override COVER: f32 = 0.0;\n"
"@vertex fn vs(@builtin(vertex_index) i: u32) -> @builtin(position) vec4f {\n"
"  var p = array<vec2f,3>(vec2f(-1.0,-3.0), vec2f(-1.0,1.0), vec2f(3.0,1.0));\n"
"  return vec4f(p[i] * COVER, 0.0, 1.0);\n"
"}\n"
"override uR: f32 = 1.0;\n"
"override uG: f32 = 0.0;\n"
"@fragment fn fs() -> @location(0) vec4f { return vec4f(uR, uG, 0.0, 1.0); }\n";
...
static void build(void) {
    ...
    /* Vertex override: COVER = 1.0 (scale the fullscreen triangle up from its
       degenerate 0.0 default). */
    WGPUConstantEntry vsConst[1];
    vsConst[0].nextInChain = NULL;
    vsConst[0].key.data = "COVER"; vsConst[0].key.length = WGPU_STRLEN;
    vsConst[0].value = 1.0;

    /* Fragment overrides: uR=0, uG=1 (green, overriding the red default). */
    WGPUConstantEntry fsConst[2];
    fsConst[0].nextInChain = NULL; fsConst[0].key.data = "uR"; fsConst[0].key.length = WGPU_STRLEN; fsConst[0].value = 0.0;
    fsConst[1].nextInChain = NULL; fsConst[1].key.data = "uG"; fsConst[1].key.length = WGPU_STRLEN; fsConst[1].value = 1.0;
    ...
    fs.constantCount = 2; fs.constants = fsConst;
    ...
    pd.vertex.constantCount = 1; pd.vertex.constants = vsConst;
...
int main(void) {
    ...
    printf("webgpu const: main done, waiting for adapter\n");
    return 0;
}
