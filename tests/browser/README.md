...
| `os-vt.mjs` | **VT switching on the OS page** (todos/0022): boot lands on VT1 (tty full-page), Ctrl+Alt+F1/F2 + the status-strip switch flip between tty and desktop (exactly one visible), shell stays usable mid-doom and after `kill`ing the wm service (kernel-chrome fallback — the availability rationale), halt forces VT1. `node os-vt.mjs`. |
