...
| `os-doom.mjs` | **Windowed vendor apps on the OS page** (todos/0015): first boot seeds doom/gameboy/snake + game data (image.json `bin` entries), then `doom &` — asserts a real composited frame (color histogram), Escape reaching the app, attract-demo motion, clean quit via `wmctl close` (DOOM's 1280x800 window clips the close box off the 800x500 desktop), and the gameboy+ROM lifecycle. `node os-doom.mjs`. |
| `os-vt.mjs` | **VT switching on the OS page** (todos/0022): boot lands on VT1 (tty full-page), the Terminal/Desktop tab bar (primary) + Ctrl+Alt+F1/F2 (alias) flip between tty and desktop (exactly one visible), shell stays usable mid-doom and after `kill`ing the wm service (kernel-chrome fallback — the availability rationale), halt forces VT1. `node os-vt.mjs`. |
