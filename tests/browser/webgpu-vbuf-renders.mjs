...
// webgpu-vbuf.c -> .html, drives it with Chrome (system channel, which
...
const PORT = 3191;
const URL = `http://localhost:${PORT}/webgpu-vbuf.html`;

const out = path.join(__dirname, 'www', 'webgpu-vbuf.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-vbuf] compiling webgpu-vbuf.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-vbuf.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-vbuf] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-vbuf] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-vbuf.png') });
  ...
  console.log('[webgpu-vbuf] PASS — pink triangle on dark-blue clear (shot-webgpu-vbuf.png)');
} catch (e) {
  console.error('[webgpu-vbuf] FAILED:', e.message);
