...
// webgpu-texview.c -> .html. The program samples a 2x2 r8unorm texture (new
...
//
// Run: node webgpu-texview-renders.mjs
...
const PORT = 3199;
const URL = `http://localhost:${PORT}/webgpu-texview.html`;

const out = path.join(__dirname, 'www', 'webgpu-texview.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-texview] compiling webgpu-texview.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-texview.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-texview] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-texview] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-texview.png') });
  ...
  if (!(center.r > 150 && center.b > 120 && center.r > center.g + 40)) {
    throw new Error(`center not pink (baseArrayLayer ignored?): ${JSON.stringify(center)}`);
  }
  if (!(corner.b > corner.r && corner.b > corner.g && corner.r < 100)) {
    throw new Error(`corner not dark-blue clear: ${JSON.stringify(corner)}`);
  }
  console.log('[webgpu-texview] PASS — 2D view of array layer 1 (baseArrayLayer honored) (shot-webgpu-texview.png)');
} catch (e) {
  console.error('[webgpu-texview] FAILED:', e.message);
