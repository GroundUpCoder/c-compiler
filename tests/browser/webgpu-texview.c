/* WebGPU texture-view descriptors (Phase A7). A 2-layer array texture is filled
   GREEN on layer 0 and PINK on layer 1. The sampled view is created with a
   descriptor selecting baseArrayLayer=1, arrayLayerCount=1, dimension=2D — a 2D
   view onto layer 1. If wgpuTextureCreateView ignores the descriptor (old stub
   behavior -> whole texture / layer 0) the center samples GREEN and fails the
   pink check. Driven by webgpu-texview-renders.mjs. */
#include <webgpu.h>
#include <stddef.h>
#include <stdio.h>
...
static WGPUTexture make_array_texture(void) {
    ...
    td.size.width = 1;
    td.size.height = 1;
    td.size.depthOrArrayLayers = 2;       /* 2 array layers */
...
static WGPUTextureView make_layer1_view(WGPUTexture t) {
    WGPUTextureViewDescriptor vd;
    zero(&vd, sizeof(vd));
    vd.dimension = WGPUTextureViewDimension_2D;
    vd.baseArrayLayer = 1;                 /* the feature under test */
...
static void build(void) {
    ...
    WGPUTextureView texView = make_layer1_view(tex);
...
int main(void) {
    ...
    printf("webgpu texview: main done, waiting for adapter\n");
    return 0;
}
