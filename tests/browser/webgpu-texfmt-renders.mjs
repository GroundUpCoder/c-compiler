...
// webgpu-texfmt.c -> .html. The program samples a 2x2 r8unorm texture (new
...
//
// Run: node webgpu-texfmt-renders.mjs
...
const PORT = 3195;
const URL = `http://localhost:${PORT}/webgpu-texfmt.html`;

const out = path.join(__dirname, 'www', 'webgpu-texfmt.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-texfmt] compiling webgpu-texfmt.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-texfmt.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-texfmt] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-texfmt] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-texfmt.png') });
  ...
  if (!(center.r > 150 && center.b > 120 && center.r > center.g + 40)) {
    throw new Error(`center not pink (r8unorm decode wrong?): ${JSON.stringify(center)}`);
  }
  if (!(corner.b > corner.r && corner.b > corner.g && corner.r < 100)) {
    throw new Error(`corner not dark-blue clear: ${JSON.stringify(corner)}`);
  }
  console.log('[webgpu-texfmt] PASS — r8unorm sample + format-map smoke (shot-webgpu-texfmt.png)');
} catch (e) {
  console.error('[webgpu-texfmt] FAILED:', e.message);
