/* WebGPU texture-format coverage (Phase A1). Two checks in one program:

   1. Correctness: a 2x2 *r8unorm* texture (a newly-added format) is written with
      wgpuQueueWriteTexture and sampled; the fragment shader expands the single
      red channel into pink (r, r*0.2, r*0.8). If r8unorm createTexture / write /
      sample decode are wrong the center stops being pink.

   2. Smoke: wgpuDeviceCreateTexture is called for a representative spread of the
      newly-added uncompressed formats (8/16/32-bit, rgb10a2, rg11b10, rgb9e5).
      Each must resolve through the host's expanded format map without the
      binding throwing "unsupported WGPUTextureFormat" — proving the map covers
      them. Prints "TEXFMT-SMOKE <n> OK".

   Driven by webgpu-texfmt-renders.mjs. */
#include <webgpu.h>
#include <stddef.h>
#include <stdio.h>
...
/* 2x2 R8, every texel full red (1.0). bytesPerRow = 2 (1 byte/texel). */
static const unsigned char texels[2 * 2] = { 255, 255, 255, 255 };
...
"  let r = textureSample(tex, samp, uv).r;\n"
"  return vec4f(r, r * 0.2, r * 0.8, 1.0);\n"   /* r8unorm -> pink */
"}\n";

static void zero(void *p, size_t n) { char *z = (char *)p; for (size_t i = 0; i < n; i++) z[i] = 0; }

/* Smoke-test the expanded format map: create + release a texture per format. */
static void smoke_formats(void) {
    static const WGPUTextureFormat fmts[] = {
        ...
        WGPUTextureFormat_RG16Float, WGPUTextureFormat_RGBA8Snorm, WGPUTextureFormat_RGBA8Uint,
        WGPUTextureFormat_RGB10A2Unorm, WGPUTextureFormat_RG11B10Ufloat, WGPUTextureFormat_RGB9E5Ufloat,
        WGPUTextureFormat_RGBA16Float, WGPUTextureFormat_RGBA16Uint,
        WGPUTextureFormat_RGBA32Float, WGPUTextureFormat_RGBA32Uint,
    };
...
static WGPUTexture make_texture(void) {
    ...
    td.size.height = 2;
    td.size.depthOrArrayLayers = 1;
    td.format = WGPUTextureFormat_R8Unorm;       /* new format under test */
    td.mipLevelCount = 1;
    td.sampleCount = 1;
    WGPUTexture t = wgpuDeviceCreateTexture(device, &td);
    ...
    layout.offset = 0;
    layout.bytesPerRow = 2;        /* 2 texels * 1 byte */
...
int main(void) {
    ...
    printf("webgpu texfmt: main done, waiting for adapter\n");
    return 0;
}
