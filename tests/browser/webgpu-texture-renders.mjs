...
// webgpu-texture.c -> .html, drives it with Chrome (system channel, which
...
const PORT = 3195;
const URL = `http://localhost:${PORT}/webgpu-texture.html`;

const out = path.join(__dirname, 'www', 'webgpu-texture.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-texture] compiling webgpu-texture.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-texture.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-texture] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-texture] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-texture.png') });
  ...
  console.log('[webgpu-texture] PASS — pink triangle on dark-blue clear (shot-webgpu-texture.png)');
} catch (e) {
  console.error('[webgpu-texture] FAILED:', e.message);
