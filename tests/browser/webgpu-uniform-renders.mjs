...
// webgpu-uniform.c -> .html, drives it with Chrome (system channel, which
...
const PORT = 3193;
const URL = `http://localhost:${PORT}/webgpu-uniform.html`;

const out = path.join(__dirname, 'www', 'webgpu-uniform.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-uniform] compiling webgpu-uniform.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-uniform.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-uniform] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-uniform] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-uniform.png') });
  ...
  console.log('[webgpu-uniform] PASS — pink triangle on dark-blue clear (shot-webgpu-uniform.png)');
} catch (e) {
  console.error('[webgpu-uniform] FAILED:', e.message);
