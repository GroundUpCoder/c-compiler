/* WebGPU buffer write-mapping (Phase A4). Same pink triangle as webgpu-vbuf.c,
   but the vertex buffer is created with mappedAtCreation=1 and filled by writing
   into wgpuBufferGetMappedRange + wgpuBufferUnmap — NO QueueWriteBuffer, NO
   CopyDst usage. If the host doesn't honor mappedAtCreation or doesn't flush the
   mapped range back to the GPU on unmap, the buffer stays zero and nothing
   renders. A pink center proves the write-mapping path. Driven by
   webgpu-mapwrite-renders.mjs. */
#include <webgpu.h>
#include <stddef.h>
#include <string.h>
#include <stdio.h>
...
static void build_buffer(void) {
    ...
    bd.label.data = NULL;
    bd.label.length = 0;
    bd.usage = WGPUBufferUsage_Vertex;     /* no CopyDst: filled via mapping */
    bd.size = sizeof(verts);
    bd.mappedAtCreation = 1;
    vbuf = wgpuDeviceCreateBuffer(device, &bd);

    void *p = wgpuBufferGetMappedRange(vbuf, 0, sizeof(verts));
    memcpy(p, verts, sizeof(verts));
    wgpuBufferUnmap(vbuf);                  /* flushes the write back to the GPU */
}
...
int main(void) {
    ...
    printf("webgpu mapwrite triangle: main done, waiting for adapter\n");
    return 0;
}
