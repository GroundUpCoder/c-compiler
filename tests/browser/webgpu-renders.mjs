...
// webgpu-triangle.c -> .html, drives it with Chrome (system channel, which
...
console.log('[webgpu] compiling webgpu-triangle.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-triangle.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-triangle.png') });
  ...
  console.log('[webgpu] PASS — pink triangle on dark-blue clear (shot-webgpu-triangle.png)');
} catch (e) {
