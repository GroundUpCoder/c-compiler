...
// compiles webgpu-msaa.c -> .html (position fed as Float16x2, color as
...
//
// Run: node webgpu-msaa-renders.mjs
...
const PORT = 3198;
const URL = `http://localhost:${PORT}/webgpu-msaa.html`;

const out = path.join(__dirname, 'www', 'webgpu-msaa.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-msaa] compiling webgpu-msaa.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-msaa.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-msaa] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-msaa] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-msaa.png') });

  const { center, corner } = s;
  if (!(center.r > 150 && center.b > 120 && center.r > center.g + 40)) {
    throw new Error(`center not pink (MSAA count or resolveTarget wrong?)))): ${JSON.stringify(center)}`);
  }
  if (!(corner.b > corner.r && corner.b > corner.g && corner.r < 100)) {
    throw new Error(`corner not dark-blue clear: ${JSON.stringify(corner)}`);
  }
  console.log('[webgpu-msaa] PASS — 4x MSAA render + resolveTarget to surface (shot-webgpu-msaa.png)');
} catch (e) {
  console.error('[webgpu-msaa] FAILED:', e.message);
