...
// webgpu-texbind.c -> .html. The program samples a 2x2 r8unorm texture (new
...
//
// Run: node webgpu-texbind-renders.mjs
...
const PORT = 3200;
const URL = `http://localhost:${PORT}/webgpu-texbind.html`;

const out = path.join(__dirname, 'www', 'webgpu-texbind.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-texbind] compiling webgpu-texbind.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-texbind.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-texbind] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-texbind] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-texbind.png') });
  ...
  if (!(center.r > 150 && center.b > 120 && center.r > center.g + 40)) {
    throw new Error(`center not pink (BGL viewDimension hardcoded 2d?): ${JSON.stringify(center)}`);
  }
  if (!(corner.b > corner.r && corner.b > corner.g && corner.r < 100)) {
    throw new Error(`corner not dark-blue clear: ${JSON.stringify(corner)}`);
  }
  console.log('[webgpu-texbind] PASS — 2d-array binding viewDimension honored (shot-webgpu-texbind.png)');
} catch (e) {
  console.error('[webgpu-texbind] FAILED:', e.message);
