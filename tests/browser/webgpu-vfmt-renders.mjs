...
// compiles webgpu-vfmt.c -> .html (position fed as Float16x2, color as
...
//
// Run: node webgpu-vfmt-renders.mjs
...
const PORT = 3194;
const URL = `http://localhost:${PORT}/webgpu-vfmt.html`;

const out = path.join(__dirname, 'www', 'webgpu-vfmt.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
console.log('[webgpu-vfmt] compiling webgpu-vfmt.c → emitted .html …');
const r = spawnSync('node', [path.join(ROOT, 'compiler.js'),
  path.join(__dirname, 'webgpu-vfmt.c'), '-o', out, '--no-version-check'], { stdio: 'inherit' });
if (r.status !== 0) { console.error('[webgpu-vfmt] compile failed'); process.exit(1); }
...
try {
  ...
  console.log('[webgpu-vfmt] sampled', JSON.stringify(s));
  await page.screenshot({ path: path.join(__dirname, 'shot-webgpu-vfmt.png') });

  const { center, corner } = s;
  if (!(center.r > 150 && center.b > 120 && center.r > center.g + 40)) {
    throw new Error(`center not pink (Float16x2/Snorm8x4 decode wrong?): ${JSON.stringify(center)}`);
  }
  if (!(corner.b > corner.r && corner.b > corner.g && corner.r < 100)) {
    throw new Error(`corner not dark-blue clear: ${JSON.stringify(corner)}`);
  }
  console.log('[webgpu-vfmt] PASS — Float16x2 + Snorm8x4 vertex formats decode (shot-webgpu-vfmt.png)');
} catch (e) {
  console.error('[webgpu-vfmt] FAILED:', e.message);
