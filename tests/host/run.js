...
var tests = [
  ...
  ['test_aggregate_frame.js', []],   // #773: aggregate-return temporaries are STATIC frame slots — the inliner's structRet refusal is removed and the callees really inline (leg 3 is the noinline red control; leg 4 pins the deferred-bump retirement, not just the lifted refusal)
  ...
  ['test_host_preflight.js', []],        // #725 stage B: the host-health preflight + mid-run truncation — critical fake refuses at exit 2 ([host-health] marker, override documented in-message, per-runId environmental refusal record, NO summary), healthy/warn/unmeasured/real-host all proceed, CC_NO_HOST_HEALTH=1 runs unguarded but recorded, and the array-fake truncation leg pins fail/'host-degraded'/DID-NOT-RUN rows with the run still summarized+archived. All degraded states are INJECTED (proxy — labeled as such)
  ...
  ['test_cli_options.js', []], // #770: honest option refusal and help
  ['test_debug_flags.js', []], // #762: metadata and inlining controls, both frontends
  ['test_trap_stack_portability.js', []], // #783: captured JSC frames keep unknown offsets unknown
  ['test_trap_backtrace.js', []],     // #759 (design #732): a real wasm trap delivers a SYMBOLICATED backtrace (function name + file:line, resolved from the name section and c.sourcemap — V8's frame offset is module-relative, the same coordinate space) to the PROCESS'S OWN fd 2, capturable by an ordinary `2>` redirect. Carries the negatives as legs: a module that compiles but cannot LINK (#558's real cause, reproduced by renaming one import in place) and any non-trap rejection (#752's never-ran class) must stay SILENT, and a clean program is the positive control that stops this file passing vacuously. Every expected name and line is derived from the fixture at run time, never harvested from the output
