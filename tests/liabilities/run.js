#!/usr/bin/env node
'use strict';
// tests/liabilities/run.js — the `liabilities` suite: the liability register validator
// and its tests (docs/archive/0286), plus the Lnn id-allocator tests.
//
// The register validator has to gate from a suite because the pre-commit hook
// is per-clone opt-in (`git config core.hooksPath tools/githooks`) — a
...
//
//   node tests/liabilities/run.js [--filter=STR]
//
...
// Cross-tree preflight (docs/archive/0341) — every case below runs with `cwd: ROOT`,
// so without this a main-tree copy launched from a worktree would validate
// MAIN's queue and register and report it as yours.
require('../lib/tree-guard.js').assertSameTree(__dirname, { label: 'tests/liabilities/run.js' });

const CASES = [
  { name: 'liabilities-check', argv: ['tools/liabilities/liabilities.js', 'check'] },
  { name: 'liabilities-tests', argv: ['tools/liabilities/liabilities.test.js'] },
  // The id allocator's freshness half (docs/archive/0360). Real clones over local
  // paths, so it stays offline and cheap like the rest of this suite.
  { name: 'idspace-tests',     argv: ['tools/liabilities/idspace.test.js'] },
];
...
function main() {
  ...
  if (!cases.length) {
    process.stdout.write(`liabilities: no case matches --filter=${filter} (have: ${CASES.map(c => c.name).join(', ')})\n`);
    process.exit(1);
  }
  ...
  process.stdout.write(`\nliabilities suite: ${cases.length - failed}/${cases.length} passed, ${failed} failed  (${secs}s)\n`);
